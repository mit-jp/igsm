...
!CAS
!
! !ADDITION: JUNE, 2004 - ADAM SCHLOSSER
! Depending on defined status of the ifdef flag "STOCHASTIC", the module will
! also read in parameter values for storm duration and inter-storm period 
! which are used in the Poisson-based stochastic precipitation generator.
! These parameter assignments are carried out in 'surfrd'.
!
!CAS
!
! !PUBLIC TYPES:
...
! !LOCAL VARIABLES:
    ...
    integer  :: nmth                               !number of input data months
...
!-----------------------------------------------------------------------
       ...
       do i=1,numlon(j)
          if (landmask(i,j) == 1) then
             ...
             do m = 1, maxpatch_pft
                ...
                !write (6,*) 'SURFRD: wtxy = ',wt(i,j,m)
                !write (6,*) 'SURFRD: pctpft = ',pctpft(i,j,m)
                !write (6,*) 'at i = ',i,' j = ',j,' for PFT = ',m
             end do
    ...
    write (6,*) 'Attempting to read stochastic parmater data.....'
    call getfil (fstochdat, locfn, 0)
    call wrap_open(locfn, 0, ncid)

    call wrap_inq_dimid  (ncid, 'lsmlon', dimid)
    call wrap_inq_dimlen (ncid, dimid, nlon_i)
    if (nlon_i /= lsmlon) then
       write(6,*)'STOCHASTIC: parameter lsmlon= ',lsmlon, &
            'does not equal input nlon_i= ',nlon_i
       call endrun
    endif
    ...
    call wrap_inq_dimlen (ncid, dimid, nlat_i)
    if (nlat_i /= lsmlat) then
       write(6,*)'STOCHASTIC: parameter lsmlat= ',lsmlat, &
            'does not equal input nlat_i= ',nlat_i
       call endrun
    endif

    call wrap_inq_dimid  (ncid, 'mth', dimid)
    call wrap_inq_dimlen (ncid, dimid, nmth)
    if (nmth /= 12) then
       write(6,*)'STOCHASTIC: parameter mth= ',nmth , &
            'does not equal # of months per year'
       call endrun
    endif

       call wrap_inq_varid (ncid, 'EXP_TSTORM', varid) 
       call wrap_get_var_realx (ncid, varid, exp_tstorm)
!      write (6,*) '----------------------------------'
!      write (6,*) 'Expected storm duration'
!      write (6,*) '----------------------------------'
!      write (6,*) exp_tstorm

       call wrap_inq_varid (ncid, 'EXP_TDRY', varid) 
       call wrap_get_var_realx (ncid, varid, exp_tdry)
!      write (6,*) '----------------------------------'
!      write (6,*) 'Expected inter-storm period'
!      write (6,*) '----------------------------------'
!      write (6,*) exp_tdry

!       call wrap_inq_varid (ncid, 'TRND_TDRY', varid) 
!       call wrap_get_var_realx (ncid, varid, trnd_tdry)
!       write (6,*) '----------------------------------'
!       write (6,*) 'Trend in inter-storm period (%/C)'
!       write (6,*) '----------------------------------'
!       write (6,*) trnd_tdry
