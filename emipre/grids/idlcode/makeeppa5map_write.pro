pro makeeppa5map_write, scale

; read in country numbers
dir='/Users/noelle/Documents/MIT07/igsm/newprocess/grids/idlcode/countries_eppa5/'
...
roe=read_ascii(dir+'roe.csv', template=countrytemplate,  /verbose)
eur=read_ascii(dir+'eur.csv', template=countrytemplate,  /verbose)
rus=read_ascii(dir+'rus.csv', template=countrytemplate,  /verbose)
bra=read_ascii(dir+'bra.csv', template=countrytemplate,  /verbose)
...
rea=read_ascii(dir+'rea.csv', template=countrytemplate,  /verbose)
...
for ii=0,  N_Elements(roe.numbers)-1L do begin
eetcells=find_cells_by_country(roe.numbers[ii], onebyone)
newmap=newmap+eetcells
endfor
openw, lun, 'eppa.roe.'+scale, /get_lun,  /f77_unformatted
newmap=double(newmap)
writeu,  lun,  newmap
close,  lun
free_lun,  lun
...
for ii=0,  N_Elements(rus.numbers)-1L do begin
fsucells=find_cells_by_country(rus.numbers[ii], onebyone)
newmap=newmap+fsucells
endfor
openw, lun, 'eppa.rus.'+scale, /get_lun,  /f77_unformatted
newmap=double(newmap)
writeu,  lun,  newmap
close,  lun
free_lun,  lun
...
for ii=0,  N_Elements(bra.numbers)-1L do begin
idzcells=find_cells_by_country(bra.numbers[ii], onebyone)
newmap=newmap+idzcells
endfor
openw, lun, 'eppa.bra.'+scale, /get_lun,  /f77_unformatted
newmap=double(newmap)
writeu,  lun,  newmap
close,  lun
free_lun,  lun
...
for ii=0,  N_Elements(rea.numbers)-1L do begin
rowcells=find_cells_by_country(rea.numbers[ii], onebyone)
newmap=newmap+rowcells
endfor
openw, lun, 'eppa.rea.'+scale, /get_lun,  /f77_unformatted
newmap=double(newmap)
writeu,  lun,  newmap
close,  lun
free_lun,  lun
