...
Biome43 veg;
Lulcdata43 vegdat;
Clmdata43 clmdat[MAXRTIME];
MaxCohortdata43 cohorts;
Temdata43 temdat[MAXRTIME];
Season pred;
...
int main() 
{
  ...
  cout << "  Enter 1 for the TEM output data format: ";

  cin >> dataflg;

  if( dataflg != 1 ) 
  {
    cout << "Enter the name of the vegetation type file: ";
  ...
  {
//    veg.getvtype( flog1 );
    cout << "Enter the name of the cohort file: ";
  ...
  for( dyr = 0; dyr < (lastyr-startyr+1); ++dyr )
  {
    switch( dataflg )
    {
      case 0: fveg = fopen( vegfname.c_str(), "r" );
      ...
      case 1: fchrt = fopen( chrtfname.c_str(), "r" );
	...
	predmap = clmdat[dyr].varname;
        ...
        if( 0 == potvflag )
        { 
          pred.tveg = vegdat.potveg;
        }
        else
        {
          pred.tveg = vegdat.currentveg;
        }
	...
	pred.carea = clmdat[dyr].carea;
	pred.subarea = pred.carea;
	...
	for( dm = 0; dm < CYCLE; ++dm ) 
      ...
      {
        if( 0 == dyr 
            || 1 == transflag && (startyr+dyr) >= firstchrtyr )
        { 
