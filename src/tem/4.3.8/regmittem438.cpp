/* *************************************************************
REGMITTEM438.CPP - Extrapolation version of the Terrestrial 
                     Ecosystem Model Version 4.3 used with 
                     interactive version of TEM (IMITTEM437.CXX)
****************************************************************
...
//  const double gCH4togC = 12.0 / 16.0;
  ...
  for( igrd = 0; igrd < mxnumgrid; ++igrd )
  {
    ...
    for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
    {
      ...
      // Determine soil characteristics like rooting depth 
...
************************************************************* */
      ...
      for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
      {
        ...
        if( istateflag > 0 )
        {
          ...
          //cout << " NONSOLC after readCohort = " << telmnt[igrd].cohort[ichrt].NEMnsolc << endl;
                               ...
                               const int& ichrt )
{
  ...
  switch ( ichrt )
  {
    case 17: tchrt = 17; break;
    case 18: tchrt = 17; break;
    case 19: tchrt = 17; break;
    case 20: tchrt = 17; break;
    case 21: tchrt = 17; break;
    case 22: tchrt = 17; break;
    case 23: tchrt = 17; break;
    case 24: tchrt = 17; break;
    case 25: tchrt = 17; break;
    case 26: tchrt =  4; break;
    case 27: tchrt = 14; break;
    case 28: tchrt =  7; break;
    case 29: tchrt = 13; break;
    case 30: tchrt = 18; break;
    case 31: tchrt = 18; break;
    case 32: tchrt = 15; break;
    case 33: tchrt = 15; break;
    case 34: tchrt = 18; break;
    default: tchrt = ichrt;
  }
  ...
  telmnt[igrd].tem.soil.setINEET( (climate4tem_.aet[igrd][tchrt]
  ...
  {
    telmnt[igrd].tem.soil.setMOIST( (climate4tem_.sh2o1m[igrd][tchrt]
                                    * telmnt[igrd].tem.soil.getROOTZ()) ); 
  }
  else if ( telmnt[igrd].tem.soil.getROOTZ() <= 2.0 )
  {
    telmnt[igrd].tem.soil.setMOIST( (climate4tem_.sh2o1m[igrd][tchrt]
                                    * (2.0 - telmnt[igrd].tem.soil.getROOTZ()))
                                    + (climate4tem_.sh2o2m[igrd][tchrt]
                                    * (telmnt[igrd].tem.soil.getROOTZ() - 1.0)) ); 
  }
  else 
  {        
    telmnt[igrd].tem.soil.setMOIST( (climate4tem_.sh2o2m[igrd][tchrt]
  ...
  { 
    for ( dday = 0; dday < telmnt[0].mitclm.mdays[pdm]; ++dday )
    {
      for ( dlyr = 0; dlyr < (CLMNLAYERS-1); ++dlyr )
      {
        telmnt[igrd].tem.microbe.nem.dayTemp[dlyr][dday] = climate4tem_.daytsoil[dlyr][igrd][tchrt][dday];

        telmnt[igrd].tem.microbe.nem.dayTemp[dlyr][dday] += KELVIN0; 
       
        telmnt[igrd].tem.microbe.nem.dayMoist[dlyr][dday] = climate4tem_.daysh2o[dlyr][igrd][tchrt][dday];
      }
    }
  }

   
  telmnt[igrd].tem.soil.setSNOWPACK( climate4tem_.swe[igrd][tchrt] );

  telmnt[igrd].tem.soil.setSURFRUN( (climate4tem_.sfr[igrd][tchrt]
                                    * telmnt[igrd].mitclm.ndays[pdm]) );

  telmnt[igrd].tem.soil.setDRAINAGE( (climate4tem_.drn[igrd][tchrt]
                                     * telmnt[igrd].mitclm.ndays[pdm]) );
       ...
       && 1 == telmnt[0].mitclm.n2oflag )
  {
    for ( dday = 0; dday < telmnt[0].mitclm.mdays[pdm]; ++dday )
    {
      for ( dlyr = 0; dlyr < NLAYERS; ++dlyr )
      {
        ...
        {
          telmnt[igrd].tem.microbe.nem.hourMoist[dlyr][dday][dhr] = climate4tem_.hrsh2o[dlyr][igrd][tchrt][dday][dhr];
        }
      }                                       
    }
  }
                                   ...
                                   FILE* flulc )
{
  ...
  while ( igrd < mxnumgrid && 0 == fatalerr )
  {
        ...
        || 1 == telmnt[0].lcluc.tlulcflag  )
    {
      ...
      if( 0 == pdyr )
      {
        if( telmnt[igrd].year != mxcohrtdat.year 
            && 1 == telmnt[0].lcluc.tlulcflag )
        { 
          cout << " Year in CLM data does not match ";
          cout << " Year in MAXCOHORTS data" << endl;
          cout << " at Lon = " << telmnt[igrd].col;
          cout << "  Lat = " << telmnt[igrd].row << endl;
          cout << "  CLM year = " << telmnt[igrd].year;
          cout << "  MAXCOHORTS year = " << mxcohrtdat.year;
          cout << endl << endl;
      
          flog1 << " Year in CLM data does not match ";
          flog1 << " Year in MAXCOHORTS data" << endl << endl;
          flog1 << " at Lon = " << telmnt[igrd].col;
          flog1 << "  Lat = " << telmnt[igrd].row << endl;
          flog1 << " CLM year = " << telmnt[igrd].year;
          flog1 << " MAXCOHORTS year = " << mxcohrtdat.year;
          flog1 << endl << endl;

          exit( -1 );
        }
      }
//      else { telmnt[igrd].year = mxcohrtdat.year; }
      ...
      if( 0 == pdyr )
      {
        telmnt[igrd].prvmxcohrts = mxcohrtdat.total;
      }
      else
      {
        telmnt[igrd].prvmxcohrts = telmnt[igrd].maxcohorts;
      }

      // Pass mxcohortdat information to telmnt[igrd]

      telmnt[igrd].maxcohorts = mxcohrtdat.total;
      telmnt[igrd].natcohorts = mxcohrtdat.natchrts;
      telmnt[igrd].carea = mxcohrtdat.carea;
      telmnt[igrd].contnent = mxcohrtdat.contnent;
      ...
      for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
      {
        ...
        if( telmnt[igrd].year != lulcdat[ichrt].year 
            && 1 == telmnt[0].lcluc.tlulcflag )
        { 
          cout << " Year in MAXCOHRTS data does not match ";
          cout << " Year in LCLUC data" << endl;
          cout << " at Lon = " << telmnt[igrd].col;
          cout << "  Lat = " << telmnt[igrd].row << endl;
          cout << "  MAXCOHORTS year = " << telmnt[igrd].year;
          cout << "  LCLUC year = " << lulcdat[ichrt].year;
          cout << " for cohort " << (ichrt+1);
          cout << endl << endl;
      
          flog1 << " Year in MAXCOHORTS data does not match ";
          flog1 << " Year in LCLUC data" << endl << endl;
          flog1 << " at Lon = " << telmnt[igrd].col;
          flog1 << "  Lat = " << telmnt[igrd].row << endl;
          flog1 << " MAXCOHORTS year = " << telmnt[igrd].year;
          flog1 << " LCLUC year = " << lulcdat[0].year;
          cout << " for cohort " << (ichrt+1);
          flog1 << endl << endl;
        ...
        telmnt[igrd].cohort[ichrt].srcCohort = lulcdat[ichrt].isrccohort;
        telmnt[igrd].cohort[ichrt].chrtarea = lulcdat[ichrt].chrtarea;
        telmnt[igrd].cohort[ichrt].potveg = lulcdat[ichrt].potveg;
        telmnt[igrd].cohort[ichrt].currentveg = lulcdat[ichrt].currentveg;
        telmnt[igrd].cohort[ichrt].subtype = lulcdat[ichrt].subtype;
        telmnt[igrd].cohort[ichrt].FRF = lulcdat[ichrt].FRF;
        telmnt[igrd].cohort[ichrt].agstate = lulcdat[ichrt].agstate;
        telmnt[igrd].cohort[ichrt].agprvstate = lulcdat[ichrt].agprevstate;
        telmnt[igrd].cohort[ichrt].tillflag = lulcdat[ichrt].tillflag;                           
        telmnt[igrd].cohort[ichrt].fertflag = lulcdat[ichrt].fertflag;                              
        telmnt[igrd].cohort[ichrt].irrgflag = lulcdat[ichrt].irrgflag;                              
        telmnt[igrd].cohort[ichrt].slashpar = lulcdat[ichrt].slashpar; 
        telmnt[igrd].cohort[ichrt].vconvert = lulcdat[ichrt].vconvert; 
        telmnt[igrd].cohort[ichrt].prod10par = lulcdat[ichrt].prod10par; 
        telmnt[igrd].cohort[ichrt].prod100par = lulcdat[ichrt].prod100par; 
        telmnt[igrd].cohort[ichrt].vrespar = lulcdat[ichrt].vrespar; 
        telmnt[igrd].cohort[ichrt].sconvert = lulcdat[ichrt].sconvert; 
        telmnt[igrd].region = lulcdat[ichrt].region;


        telmnt[igrd].cohort[ichrt].cmnt = telmnt[0].lcluc.getCommunityType( lulcdat[ichrt].subtype );

        telmnt[igrd].cohort[ichrt].agcmnt = telmnt[igrd].cohort[ichrt].cmnt;
    
        if( pdyr > 0 && 1 == lulcdat[ichrt].agstate )
        {
          telmnt[igrd].cohort[ichrt].agcmnt = telmnt[igrd].lcluc.getCommunityType( lulcdat[ichrt].currentveg );
        }
      }
    }  
                                    ...
                                    ifstream& ifo3 )
{
  ...
  //   4 - 16.5 to 29.0 cm
  //   5 - 29.0 to 49.4 cm 
  MITDayChrtData43 tdaySoilMoist[CLMNLAYERS-1][MAXMDAYS];

  MITDayClmData43 tdayTair[MAXMDAYS];
  ...
  //   4 - 16.5 to 29.0 cm
  //   5 - 29.0 to 49.4 cm      
  MITDayChrtData43 tdayTsoil[CLMNLAYERS-1][MAXMDAYS];     

  MITCLMdata43 tdrain2D;
  ...
  MITDayClmData43 trainDuration[MAXMDAYS];

  // Daily rain intensity of current event
  MITDayClmData43 trainIntensity[MAXMDAYS];

  MITCLMdata43 tsrfrun2D;

  // Soil moisture (mm) in top 1 meter
  MITCLMdata43 tsh2o1m2D;

  // Soil moisture (mm) in top 2 meters
  MITCLMdata43 tsh2o2m2D;
          
  MITCLMdata43 tspack2D;
       ...
       || 1 == telmnt[0].mitclm.n2oflag) )
  {
    ...
    for( dlyr = 0; dlyr < (CLMNLAYERS-1); ++dlyr )
    {
      for( dgrd = 0; dgrd < MAXNGRD; ++dgrd )
      {
        for( dchrt = 0; dchrt < MXNCHRTS; ++dchrt )
        {
          for( dday = 0; dday < telmnt[0].mitclm.mdays[pdm]; ++dday )
          {
            climate4tem_.daytsoil[dlyr][dgrd][dchrt][dday] = 
                     tdayTsoil[dlyr][dday].latband[dgrd][dchrt];

            climate4tem_.daysh2o[dlyr][dgrd][dchrt][dday] = 
                     tdaySoilMoist[dlyr][dday].latband[dgrd][dchrt];
          }
        }
      }
    }
      ...
      for( dgrd = 0; dgrd < MAXNGRD; ++dgrd )
      {
        for( dchrt = 0; dchrt < MXNCHRTS; ++dchrt )
        {
          for( dday = 0; dday < telmnt[0].mitclm.mdays[pdm]; ++dday )
          {
            ...
            {
              climate4tem_.hrsh2o[dlyr][dgrd][dchrt][dday][dhr] = 
                      thrSoilMoist[dlyr][dday][dhr].latband[dgrd][dchrt];
            }
          }
        }
      }
  ...
  for( dgrd = 0; dgrd < MAXNGRD; ++dgrd )
  {
    for( dchrt = 0; dchrt < MXNCHRTS; ++dchrt )
    {
      climate4tem_.aet[dgrd][dchrt] = teet2D.latband[dgrd][dchrt];
      climate4tem_.sh2o1m[dgrd][dchrt] = tsh2o1m2D.latband[dgrd][dchrt];
      climate4tem_.sh2o2m[dgrd][dchrt] = tsh2o2m2D.latband[dgrd][dchrt];
      climate4tem_.swe[dgrd][dchrt] = tspack2D.latband[dgrd][dchrt];
      climate4tem_.sfr[dgrd][dchrt] = tsrfrun2D.latband[dgrd][dchrt];
      climate4tem_.drn[dgrd][dchrt] = tdrain2D.latband[dgrd][dchrt];
    }
  }
