...
20150428 - DWK changed include mitelm44c.h to mitelm44d1.h
20150428 - DWK modified setGIStopography()
20150428 - DWK deleted elev from temgisqcin()
...
//  output[tem.I_INEET][pchrt][pdm] = tem.soil.getINEET();
  ...
  else
  {
    ...
    output[tem.I_NATLTRC][pchrt][pdm] = tem.getY( tem.I_LTRC );
  ...
  output[tem.I_NIRR][pchrt][pdm] = tem.atms.getNIRR(); 
...
void MITelmnt44::setGIStopography( const double& ppctsand,
                                   ...
                                   double pKsat[CLMNLAYERS] )                                   
{
  ...
  tem.soil.setPCTSAND( ppctsand );

  tem.soil.setPCTSILT( ppctsilt );

  tem.soil.setPCTCLAY( ppctclay );
  ...
  if( 1 == tem.ch4flag || 1 == tem.n2oflag )
  {
    ...
    {
      tem.soil.layerThick[dlyr] = pthickness[dlyr];

      tem.soil.porosity[dlyr] = pporosity[dlyr];

      tem.soil.density[dlyr] = (1.0 - tem.soil.porosity[dlyr]) * 2.7000;

      tem.soil.Ksat[dlyr] = pKsat[dlyr];
    }
...

{
  ...
  if( nirr <= -1.0 ) { return qc = 5; }

  if( par <= -1.0 ) { return qc = 6; }

  if( tair <= -99.0 ) { return qc = 7; }

  if( mxtair < -1.0 ) { return qc = 8; }

  if( yrprec <= ZERO ) { return qc = 9; }

  if( prec <= -1.0 ) { return qc = 10; }

  if( pet <= -1.0 ) { return qc = 11; }
