/* *************************************************************
REGMITTEM44D1.CPP - Extrapolation version of the Terrestrial 
...
20111020 - DWK deleted mapping of igsmveg to clmveg in 
           ...
           types
20140828 - DWK changed include regmittem44c.h to regmittem44c1.h 
20140828 - DWK changed landcover4_.landFrac to landcover4_.landArea
20140828 - DWK changed landcover4_.fracLandArea[MAXNGRD][CLMMXNVEG] 
           to landcover4_.cohortArea[MAXNGRD][CLMMXNVEG] 
20140828 - DWK changed double landareadat.landFrac to 
           landareadat.landArea
20140828 - DWK changed lulcdat[ichrt].fracLandArea to 
           lulcdat[ichrt].cohortArea
20150429 - DWK changed include from preproc.h to preproc.hpp
20150429 - DWK  telmnt[igrd].setGIStopography() modified in
           initializeMITTEMregion()
                        
****************************************************************
************************************************************* */

// NOTE: If running TEM "offline", make sure the compiler
//         directive STANDALONE_TEM is DEFINED in the preproc.hpp
//         file below.  If coupling TEM to the IGSM, make sure
//         STANDALONE_TEM is NOT DEFINED (i.e. commented out)
...
//  const double gCH4togC = 12.0 / 16.0;
  ...
  double soilLayerSH2O[CLMNLAYERS];
  ...
  for( igrd = 0; igrd < mxnumgrid; ++igrd )
  {
    ...
    telmnt[igrd].natcohorts = MAXCHRTS - 3;

    telmnt[igrd].carea = landcover4tem_.landArea[igrd];
        ...
        || (istateflag > 0 && (CYCLE-1) == pdm) )
    {
      ...
      telmnt[igrd].setGIStopography( landcover4tem_.pctsand[igrd],
                                     landcover4tem_.pctsilt[igrd],
                                     landcover4tem_.pctclay[igrd],
                                     landcover4tem_.pH[igrd],
                                     landcover4tem_.thickness[igrd],
                                     landcover4tem_.porosity[igrd],
                                     landcover4tem_.Ksat[igrd] );
      ...
      // Initialize TEMSTATE to zero for all cohorts
      
      for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
      { 
        telmnt[igrd].initializeCohortTEMState( ichrt );
      }
    }
    ...
    for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
    {
      ...
      telmnt[igrd].initAET[ichrt][pdm] = telmnt[igrd].tem.soil.getEET();
...
************************************************************* */
        ...
        for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
        {
          ...
          if( 1 == istateflag )
          {
            telmnt[igrd].readBinaryCohortState( ifstate, ichrt );
          }
...
//                                                                   dlyr );
    ...
    {      	    
      ...
      for( igrd = 0; igrd < mxnumgrid; ++igrd )
      {
        for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
        {
          ...
          telmnt[igrd].cohort[ichrt].chrtarea = landcover4tem_.cohortArea[igrd][ichrt];  
 ...
 /*         if( 1 == ostateflag )
          {
            telmnt[igrd].writeBinaryCohortState( ofstate, ichrt );
          }
          if( 2 == ostateflag )
          {
...
void RegMITTEM::setInTEMState( void )
{
  ...
  if( 0 == equil )
  {
    ...
    cout << " state of TEM at the end of a specified year:" << endl;
                               ...
                               const int& ichrt )
{
  ...
  static const double KELVIN0 = 0.00;
#else
  ...
  telmnt[igrd].tem.soil.setEET( (climate4tem_.aet[igrd][igsmveg]
                                  * telmnt[igrd].mitclm.ndays[pdm]) );
           
  if ( telmnt[igrd].tem.soil.getEET() < ZERO )
  {
    telmnt[igrd].tem.soil.setEET( ZERO );
  }    
  ...
  telmnt[igrd].tem.atms.setYRPREC( telmnt[igrd].yrprec );
                                ...
                                const long& pigrd )
{
  ...
  {       
    ...
    telmnt[pigrd].cohort[ichrt].chrtarea = landcover4tem_.cohortArea[pigrd][ichrt];  
  }
...
#ifdef STANDALONE_TEM
void RegMITTEM::setMITSoilCharacterRegion( void )
{
...
//  tem.elev = elv.elev;
  ...
  //  while ( dgrd < MAXNGRD && 0 == fatalerr )
  while ( dgrd < mxnumgrid && 0 == fatalerr )
  {
    // Read in soil characteristics data from files

    gisend = fao.getdel( fstxt );

    if( -1 == gisend )
    {
      flog1 << "Ran out of Soil texture data" << endl << endl;

      exit( -1 );
    }

//    fatalerr = coregerr( flog1,
//                         "Climate",
//                         col,
//                         row,
//                         "TEXTURE",
...
//                         fao.row );
    ...
    landcover4tem_.pctsand[dgrd] = fao.pctsand;

    landcover4tem_.pctsilt[dgrd] = fao.pctsilt;

    landcover4tem_.pctclay[dgrd] = fao.pctclay;

    landcover4tem_.pH[dgrd] = fao.pH;
        ...
        || 1 == telmnt[0].tem.n2oflag )
    {
      for( dlyr = 0; dlyr < CLMNLAYERS; ++dlyr )
      {
        ...
        if( -1 == gisend )
        {
          flog1 << "Ran out of Soil layer data" << endl << endl;

          exit( -1 );
        }

        landcover4tem_.thickness[dgrd][dlyr] = clmsoil.thickness;

        landcover4tem_.porosity[dgrd][dlyr] = clmsoil.porosity;

        landcover4tem_.Ksat[dgrd][dlyr] = clmsoil.Ksat;
      }
    }
...
void RegMITTEM::setOutTEMState( void )
{
  ...
  cout << " a specified year as binary data:" << endl;
  ...
  flog1 << " a specified year as binary data:" << endl;
...
/*long RegMITTEM::skipRecords( void )
{
  ...
  if( 0 == elmnt.strtflag )
  {
    ...
    {       
      if( 1 == temflag )
      {
        telmnt[0].setGIStopography( elmntcnt );
      }      
    }
  }
...
//      telmnt[igrd].row = mxcohrtdat.row;
      ...
      landcover4tem_.landArea[igrd] = landareadat.landArea;
  ...
  while ( igrd < MAXNGRD && 0 == fatalerr )
  {
        ...
        || 1 == telmnt[0].lcluc.tlulcflag  )
    {
      ...
      for( ichrt = 0; ichrt < telmnt[igrd].maxcohorts; ++ichrt )
      {
        ...
        landcover4tem_.cohortArea[igrd][ichrt] = lulcdat[ichrt].cohortArea;
      }      
    }  
...
/*      if( istateflag > 0 && 1 == pdyr )
      ...
      if( pdm == (CYCLE -1)  )
          ...
          telmnt[igrd].tem.veg.updateC2N( telmnt[igrd].cohort[ichrt].cmnt,
                                          telmnt[igrd].tem.soil.getYREET(),
                                          telmnt[igrd].tem.atms.getYRPET(),
...
//      cout << "ichrt = " << (ichrt+1) << endl;
      ...
      if( pdm == (CYCLE-1) )
      {
        if( telmnt[igrd].tem.totyr == ostateyear )
        {
          if( 1 == ostateflag )
          {
            telmnt[igrd].writeBinaryCohortState( ofstate, ichrt );
