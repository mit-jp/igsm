...
! !PUBLIC MEMBER FUNCTIONS:
  public :: DynEcosystemDyn       ! Ecosystem dynamics: phenology, vegetation
  public :: DynEcosystemDynini    ! Dynamically allocate memory
  public :: interpDynMonthlyVeg   ! interpolate dynamic monthly vegetation data
                                  ! used if dynamic_pft is True
!
! !REVISION HISTORY:
! based on STATICEcosysDynMod.F90 by Mariana Vertenstein
! Updated by Erwan Monier in order to read LAI, SAI, HEIGHT_TOP and HEIGHT_BOT
! varying on a monthly and annual basis.
! used if dynamic_pft is True
!
!EOP
!
! !PRIVATE MEMBER FUNCTIONS:
  private :: readDynMonthlyVegetation   ! read dynamic monthly vegetation data for two months
                                        ! used if dynamic_pft is True
!
! PRIVATE TYPES:
...
!BOP
!
! !IROUTINE: DynEcosystemDynini
!
! !INTERFACE:
  subroutine DynEcosystemDynini ()
!
! !DESCRIPTION:
...
!-----------------------------------------------------------------------
    ...
    if (ier /= 0) then
       write (6,*) 'DynEcosystemDynini allocation error'
       call endrun
    end if
  ...
  end subroutine DynEcosystemDynini

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: DynEcosystemDyn
!
! !INTERFACE:
  subroutine DynEcosystemDyn(lbp, ubp, num_nolakep, filter_nolakep, doalb)
!
! !DESCRIPTION:
...
! 03/03/11: don't reset lai of crops - use default lsm tlai/elai
  ...
  end subroutine DynEcosystemDyn

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: interpDynMonthlyVeg
!
! !INTERFACE:
  subroutine interpDynMonthlyVeg ()
!
! !DESCRIPTION:
...
! LOCAL VARIABLES:
    ...
    integer :: years(2)    ! years to be interpolated (1 to 12)
...
!-----------------------------------------------------------------------
    ...
    if (rampYear_dynpft /= 0) then
       years(1) = rampYear_dynpft
       years(2) = rampYear_dynpft
    end if

    if (InterpMonths1 /= months(1)) then
       call readDynMonthlyVegetation (fpftdyn, kyr, kmo, kda, months, years)
       InterpMonths1 = months(1)
    end if

  end subroutine interpDynMonthlyVeg

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: readDynMonthlyVegetation
!
! !INTERFACE:
  subroutine readDynMonthlyVegetation (fveg, kyr, kmo, kda, months, years)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    ...
    integer, intent(in) :: years(2)       ! years to be interpolated (0, ...)
!
! !REVISION HISTORY:
! Based on readMonthlyVegetation by Sam Levis
! Updated by Erwan Monier
!
!EOP
!
! LOCAL VARIABLES:
    ...
    integer :: g,n,i,j,k,l,m,p,ivt,yr     ! indices
    integer :: ncid,dimid,varid           ! input netCDF id's
    integer :: beg5d(5),len5d(5)          ! netCDF variable edges
    integer :: nyears                       ! number of input data year samples
    ...
    integer :: indyears(2)    ! indices of years to be interpolated (0, ...)
    ...
    integer, pointer :: yearslai(:)  ! temp local array
    character(len=32) :: subname = 'readDynMonthlyVegetation'
!-----------------------------------------------------------------------
    ...
    do k=1,2   !loop over months and read vegetated data
       ...
       if (masterproc) then
          ...
          write (6,*) 'nstep = ',get_nstep(),' year = ',kyr,' month = ',kmo,'day = ',kda

          call getfil(fveg, locfn, 0)
          call check_ret(nf_open(locfn, 0, ncid), subname)

          call check_ret(nf_inq_dimid(ncid, 'year', varid), subname)
          call check_ret(nf_inq_dimlen(ncid, varid, nyears), subname)
       ...
       call clmmpi_bcast(nyears,1,CLMMPI_INTEGER,0,clmmpicom,ier)

       allocate (yearslai(nyears), stat=ier)
       if (ier /= 0) then
       ...
       if (masterproc) then
           ...
           call check_ret(nf_get_var_int(ncid, varid, yearslai), subname)
       end if

       call clmmpi_bcast(yearslai,nyears,CLMMPI_INTEGER,0,clmmpicom,ier)

       if (years(k) <= yearslai(1)) then
          indyears(k) = 1
          found = .true.
       else if (years(k) >= yearslai(nyears)) then
          indyears(k) = nyears
          found = .true.
       else
          found = .false.
          do yr = 1,nyears-1
             if (years(k) == yearslai(yr)) then
                indyears(k) = yr
                found = .true.
             end if
          end do
...
! the current
       ...
       if (single_column) then
          ...
          beg5d(1) = closelonidx ; len5d(1) = 1
          beg5d(2) = closelatidx ; len5d(2) = 1
          beg5d(3) = 1           ; len5d(3) = npft_i
          beg5d(4) = months(k)   ; len5d(4) = 1
          beg5d(5) = indyears(k) ; len5d(5) = 1
       else
          beg5d(1) = 1           ; len5d(1) = nlon_i
          beg5d(2) = 1           ; len5d(2) = nlat_i
          beg5d(3) = 1           ; len5d(3) = npft_i
          beg5d(4) = months(k)   ; len5d(4) = 1
          beg5d(5) = indyears(k) ; len5d(5) = 1
       endif
       ...
       do n = nps,npe
          ...
          if (masterproc) then
             ...
             beg5d(4) = months(k)   ; len5d(4) = 1
             beg5d(5) = indyears(k) ; len5d(5) = 1
             if (nps == 0) beg5d(3) = n+1     ! shift by "1" if index starts at zero
             if (single_column) then
                beg5d(1) = closelonidx; len5d(1) = 1
                beg5d(2) = closelatidx; len5d(2) = 1
             else
             ...
             call check_ret(nf_get_vara_double(ncid, varid, beg5d, len5d, arrayg), subname)
          endif
  ...
  end subroutine readDynMonthlyVegetation

#endif

end module DYNPFTEcosysDynMod
