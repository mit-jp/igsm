...
! call shr_ncio_descripRead( ncId, nVars, clmmpicom, MasterTask, DescripVar )
...
! FileType="glob avg T file"
! call shr_ncio_open( NCFileName, MasterTask, FileType, ncId, exists )
! call shr_ncio_descripWrite( ncId, nVars, clmmpicom, MasterTask, exists, DescripVar )
...
SUBROUTINE shr_ncio_descripRead( ncId, nVars, prefix, clmmpicom, MasterTask, var )
...
! !INPUT/OUTPUT PARAMETERS:
  ...
  integer,          intent(IN), optional :: CLMMPICom     ! CLMMPI communicator
...
!-------------------------------------------------------------------------------
  ...
  ! --- If CLMMPI broadcast to other CLMMPI tasks -----------------------------------
  if ( present(clmmpicom) )then
     if ( debugLevel > 1 ) write(6,*) 'Broadcast variables to all tasks'
     do i = 1, nVars
        if (      var(i)%XType == shr_ncio_stringDataValue )then
           call shr_clmmpi_bcast( var(i)%StringData, clmmpicom )
        else if ( var(i)%XType == shr_ncio_integerDataValue )then
           call shr_clmmpi_bcast( var(i)%IntegerData, clmmpicom )
        else if ( var(i)%XType == shr_ncio_realR8DataValue )then
           call shr_clmmpi_bcast( var(i)%RealR8Data, clmmpicom )
        else if ( var(i)%XType == shr_ncio_logicalDataValue )then
           call shr_clmmpi_bcast( var(i)%LogicalData, clmmpicom )
        else
...
SUBROUTINE shr_ncio_descripWrite( ncId, nVars, prefix, clmmpicom, MasterTask, exists, &
                           var )
