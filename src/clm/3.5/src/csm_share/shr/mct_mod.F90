...
! !USES:
   ...
   use shr_clmmpi_mod          ! clmmpi layer
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   integer(IN)    ,intent(in),optional  :: comm  ! CLMMPI communicator
...
!-------------------------------------------------------------------------------
   ...
   if (present(comm)) then
     ...
     comm_loc = comm
     if (.not.PEOK) then
       call shr_clmmpi_commrank(comm,pe_loc,subName)
       peOK = .true.
     endif
   endif
   ...
   if (flag >= 2) then
     ...
     if (flag >= 4 .and. commOK) then
       ...
       ming = 0._R8
       maxg = 0._R8
       call shr_clmmpi_min(minl,ming,comm,subName)
       call shr_clmmpi_max(maxl,maxg,comm,subName)
     endif
...
subroutine mct_sMatP_initnc(sMatP,gsMapX,gsMapY,ConfigFileName, &
                           MapLabel,MapTypeLabel,clmmpicom)
...
!EOP
   ...
   call shr_clmmpi_commrank(clmmpicom,pe_loc)

   write(6,F00) "Initializing SparseMatrixPlus"
   call I90_allLoadF(ConfigFileName,0,clmmpicom,iret)
      if(iret /= 0) then
...
!-------------------------------------------------------------------------------
 ...
 call shr_clmmpi_commsize(mycomm,commsize)
 if (my_id == 0) then
   ...
   call shr_clmmpi_bcast(ns,mycomm,subName//" CLMMPI in ns bcast")
   call shr_clmmpi_bcast(na,mycomm,subName//" CLMMPI in na bcast")
   call shr_clmmpi_bcast(nb,mycomm,subName//" CLMMPI in nb bcast")
   ...
   do n = 1,nread
      ...
      call shr_clmmpi_bcast(Sbuf,mycomm,subName//" CLMMPI in Sbuf bcast")
      call shr_clmmpi_bcast(Rbuf,mycomm,subName//" CLMMPI in Rbuf bcast")
      call shr_clmmpi_bcast(Cbuf,mycomm,subName//" CLMMPI in Cbuf bcast")
