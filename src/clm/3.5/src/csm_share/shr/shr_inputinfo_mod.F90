...
! shr_inputInfo_initRead( NLFileName, LogPrint=.true., clmmpicom=clmmpicom, &
...
! shr_inputInfo_initRPointerRead( restart_file, CLMMPICom, MasterTask, &
!                                 CCSMInitOut )  ! Also gets file from archive
! shr_inputInfo_initRestRead( restart_file, CLMMPICom, MasterTask, &
!                             CCSMInitOut )
...
! shr_inputInfo_initRPointerWrite( ymd, tod, CLMMPICom, MasterTask, &
!                                  CCSMInit, restart_file )
! shr_inputInfo_initRestWrite( restart_file, CLMMPICom, MasterTask, &
!                              CCSMInit )
...
!EOP
    ...
    logical :: init                           ! If CLMMPI initialized or not
...
SUBROUTINE shr_inputInfo_initRead( NLFilename, LogPrint, clmmpicom, masterTask, &
                                   CCSMInitOut )
...
! !USES:
   ...
   USE shr_clmmpi_mod,    ONLY : shr_clmmpi_bcast
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   integer, optional,           intent(IN) :: clmmpicom          ! CLMMPI communicator
   logical, optional,           intent(IN) :: MasterTask      ! If CLMMPI master task
...
!EOP
    ...
    logical                    :: MasterTask2        ! If CLMMPI master task
...
!-------------------------------------------------------------------------------
    ...
    if (       present(MasterTask) .and. .not. present(clmmpicom) )then
       call shr_sys_abort( subname//':: if MasterTask input, so must clmmpicom' )
    endif
    if ( .not. present(MasterTask) .and.      present(clmmpicom) )then
       call shr_sys_abort( subname//':: if clmmpicom input, so must MasterTask' )
    endif
    !---------------------------------------------------------------------------
    ...
    !---------------------------------------------------------------------------
    ! If CLMMPI broadcast values to all processors
    !---------------------------------------------------------------------------
    if ( present(clmmpicom) )then
       call shr_clmmpi_bcast( case_desc,             CLMMPICom )
       call shr_clmmpi_bcast( case_name,             CLMMPICom )
       call shr_clmmpi_bcast( start_type,            CLMMPICom )
       call shr_clmmpi_bcast( archive_dir,           CLMMPICom )
       call shr_clmmpi_bcast( atm_ideal_phys,        CLMMPICom )
       call shr_clmmpi_bcast( atm_adiabatic,         CLMMPICom )
       call shr_clmmpi_bcast( aqua_planet,           CLMMPICom )
       call shr_clmmpi_bcast( single_column,         CLMMPICom )
       call shr_clmmpi_bcast( scmlat,                CLMMPICom )
       call shr_clmmpi_bcast( scmlon,                CLMMPICom )
       call shr_clmmpi_bcast( mss_irt,               CLMMPICom )
       call shr_clmmpi_bcast( mss_wpass,             CLMMPICom )
       call shr_clmmpi_bcast( restart_pfile,         CLMMPICom )
       call shr_clmmpi_bcast( restart_file,          CLMMPICom )
       call shr_clmmpi_bcast( restart_file_override, CLMMPICom )
       call shr_clmmpi_bcast( brnch_retain_casename, CLMMPICom )
       call shr_clmmpi_bcast( logFilePostFix,        CLMMPICom )
       call shr_clmmpi_bcast( outPathRoot,           CLMMPICom )
    end if
    !---------------------------------------------------------------------------
...
SUBROUTINE shr_inputInfo_initRestRead( restart_file, CLMMPICom, MasterTask, &
                                          CCSMInitOut )
...
!-------------------------------------------------------------------------------
  ...
  if ( present(CLMMPICom) )then
     call shr_ncio_descripRead( ncId, NVars, prefix, CLMMPICom, MasterTask2, &
                         var=CCSMInitOut%var )
  else
...
SUBROUTINE shr_inputInfo_initRestWrite( restart_file, CLMMPICom, MasterTask, &
                                           CCSMInit )
! !USES:
  use netcdf
...
!-------------------------------------------------------------------------------
  ...
  if ( present(CLMMPICom) )then
     call shr_ncio_descripWrite( ncId, NVars, prefix, clmmpicom, MasterTask2, &
...
SUBROUTINE shr_inputInfo_initRPointerRead( CLMMPICom, MasterTask, CCSMInit, &
                                           restart_file )
! !USES:
...
!-------------------------------------------------------------------------------
  ...
  if ( shr_inputInfo_initIsContinue( CCSMInit ) )then
     ...
     ! --- If CLMMPI broadcast archive filename to all tasks -------
     if ( present(CLMMPICom) ) call shr_clmmpi_bcast( rest_file_archive, CLMMPICom )
  else
...
SUBROUTINE shr_inputInfo_initRPointerWrite( ymd, tod, CLMMPICom, MasterTask, &
...
! !INPUT/OUTPUT PARAMETERS:
  ...
  logical, intent(IN) :: MasterTask                         ! If CLMMPI master task
  type(shr_inputInfo_InitType), intent(IN) :: CCSMInit      ! Input CCSM structure
