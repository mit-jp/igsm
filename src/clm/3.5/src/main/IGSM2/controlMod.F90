...
!CAS
!     o fstochdat       = 256 character stochastic precipitation parameter data file name
!     o fpcp2pft        = 256 character CLM/IGSM2 preciptiation distribution data file name
!CAS
!
    ...
    if (masterproc) then
       ...
       if (dynamic_pft) then
          ...
          if (rampYear_dynpft /= 0) then
             write(6,*)'but ramped at year: ',rampYear_dynpft
          end if
          fsurdat = fpftdyn
       else
          write(6,*)'Dynamic pft is set to FALSE'
          if (fpftdyn /= ' ') then
             call endrun('Dynamic pft is set to FALSE, but a dynamic pft file is defined (fpftdyn) in the namelist')
          end if
          write(6,*)'Using fixed pft dataset'
       end if
...
!-----------------------------------------------------------------------
    ...
    call clmmpi_bcast (caseid, len(caseid), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (ctitle, len(ctitle), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (nsrest,           1, CLMMPI_INTEGER  , 0, clmmpicom, ier)

    call clmmpi_bcast (dtime    , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)

#if (defined OFFLINE) || (defined COUP_CSM)
    call clmmpi_bcast (nestep   , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (nelapse  , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (start_ymd, 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (start_tod, 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (stop_ymd , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (stop_tod , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (ref_ymd  , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (ref_tod  , 1, CLMMPI_INTEGER  , 0, clmmpicom, ier)
    call clmmpi_bcast (calendar ,len(calendar), CLMMPI_CHARACTER, 0, clmmpicom, ier)
#endif

    ! initial file variables

    call clmmpi_bcast (nrevsn  , len(nrevsn)  , CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (finidat , len(finidat) , CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fsurdat , len(fsurdat) , CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fatmgrid, len(fatmgrid), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fatmlndfrc,len(fatmlndfrc),CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fatmtopo, len(fatmtopo) ,CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (flndtopo, len(flndtopo) ,CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fndepdat, len(fndepdat), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fndepdyn, len(fndepdyn), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (fpftdyn , len(fpftdyn) , CLMMPI_CHARACTER, 0, clmmpicom, ier)
#if (defined RTM)
    call clmmpi_bcast (frivinp_rtm, len(frivinp_rtm), CLMMPI_CHARACTER, 0, clmmpicom, ier)
#endif
    ...
    call clmmpi_bcast (irad        , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (csm_doflxave, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
    call clmmpi_bcast (rtm_nsteps  , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (nsegspc     , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (wrtdia      , 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
    call clmmpi_bcast (single_column,1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
    call clmmpi_bcast (scmlat,       1, CLMMPI_REAL8,   0, clmmpicom, ier)
    call clmmpi_bcast (scmlon,       1, CLMMPI_REAL8,   0, clmmpicom, ier)

    ! history file variables

    call clmmpi_bcast (hist_empty_htapes, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_dov2xy, size(hist_dov2xy), CLMMPI_LOGICAL, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_nhtfrq, size(hist_nhtfrq), CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_mfilt, size(hist_mfilt), CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_ndens, size(hist_ndens), CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_crtinic, len(hist_crtinic), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_avgflag_pertape, size(hist_avgflag_pertape), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_type1d_pertape, max_namlen*size(hist_type1d_pertape), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fexcl1, max_namlen*size(hist_fexcl1), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fexcl2, max_namlen*size(hist_fexcl2), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fexcl3, max_namlen*size(hist_fexcl3), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fexcl4, max_namlen*size(hist_fexcl4), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fexcl5, max_namlen*size(hist_fexcl5), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fexcl6, max_namlen*size(hist_fexcl6), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fincl1, (max_namlen+2)*size(hist_fincl1), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fincl2, (max_namlen+2)*size(hist_fincl2), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fincl3, (max_namlen+2)*size(hist_fincl3), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fincl4, (max_namlen+2)*size(hist_fincl4), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fincl5, (max_namlen+2)*size(hist_fincl5), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (hist_fincl6, (max_namlen+2)*size(hist_fincl6), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (rest_flag, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)

    ! restart file variables

    call clmmpi_bcast (rpntpath, len(rpntpath), CLMMPI_CHARACTER, 0, clmmpicom, ier)

    ! clump decomposition variables

    call clmmpi_bcast (clump_pproc, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)

    ! long term archiving variables

    call clmmpi_bcast (mss_irt, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (mss_wpass, len(mss_wpass), CLMMPI_CHARACTER, 0, clmmpicom, ier)
    call clmmpi_bcast (archive_dir, len(archive_dir), CLMMPI_CHARACTER, 0, clmmpicom, ier)

    ! error growth perturbation limit
    call clmmpi_bcast (pertlim, 1, CLMMPI_REAL8, 0, clmmpicom, ier)

#if (defined CASA)
    call clmmpi_bcast (lnpp  , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (lalloc, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (spunup, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (q10   , 1, CLMMPI_REAL8  , 0, clmmpicom, ier)
#endif
