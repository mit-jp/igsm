...
!CAS
!CAS	Assign local variables for PCP2PFT execution
!CAS
#if (defined PCP2PFT)
  real    :: dtime
  integer :: sec
...
! !USES:
  ...
  use spmdMod    , only : masterproc, npes, CLMMPI_REAL8, CLMMPI_ANY_SOURCE, &
                          CLMMPI_STATUS_SIZE, clmmpicom
...
! !LOCAL VARIABLES:
  ...
  integer :: status(CLMMPI_STATUS_SIZE) ! clmmpi status
!------------------------------------------------------------------------
  ...
  if (wrtdia) then
     ...
     call t_barrierf('sync_write_diag', clmmpicom)
     psum = sum(clm_l2a%t_rad(begg:endg))
     if (masterproc) then
        tsum = psum
        do p = 1, npes-1
           call clmmpi_recv(psum, 1, CLMMPI_REAL8, p, 999, clmmpicom, status, ier)
           if (ier/=0) then
              write(6,*) 'write_diagnostic: Error in clmmpi_recv()'
              call endrun
           end if
           tsum = tsum + psum
        end do
     else
        call clmmpi_send(psum, 1, CLMMPI_REAL8, 0, 999, clmmpicom, ier)
        if (ier/=0) then
           write(6,*) 'write_diagnostic: Error in clmmpi_send()'
           call endrun
        end if
     end if
     if (masterproc) then
