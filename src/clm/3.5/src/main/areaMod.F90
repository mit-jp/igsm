...
!------------------------------------------------------------------------
    ...
    call clmmpi_reduce(nwts,nmin,1,CLMMPI_INTEGER,CLMMPI_MIN,0,clmmpicom,ier)
    if (ier /= 0) then
       write(6,*)'mpi_reduce nwts error: ',ier
    endif
    call clmmpi_reduce(nwts,nmax,1,CLMMPI_INTEGER,CLMMPI_MAX,0,clmmpicom,ier)
    if (ier /= 0) then
       write(6,*)'mpi_reduce nwts error: ',ier
    endif
    call clmmpi_reduce(nwts,nsum,1,CLMMPI_INTEGER,CLMMPI_SUM,0,clmmpicom,ier)
    if (ier /= 0) then
       write(6,*)'mpi_reduce nwts error: ',ier
    endif
    if (masterproc) then
    ...
    imin = minval(src)
    imax = maxval(src)
    call clmmpi_reduce(imin,nmin,1,CLMMPI_INTEGER,CLMMPI_MIN,0,clmmpicom,ier)
    if (ier /= 0) then
       write(6,*)'mpi_reduce src min error: ',ier
    endif
    call clmmpi_reduce(imax,nmax,1,CLMMPI_INTEGER,CLMMPI_MAX,0,clmmpicom,ier)
    if (ier /= 0) then
    ...
    rmin = minval(wts)
    rmax = maxval(wts)
    call clmmpi_reduce(rmin,smin,1,CLMMPI_REAL8,CLMMPI_MIN,0,clmmpicom,ier)
    if (ier /= 0) then
       write(6,*)'mpi_reduce wts min error: ',ier
    endif
    call clmmpi_reduce(rmax,smax,1,CLMMPI_REAL8,CLMMPI_MAX,0,clmmpicom,ier)
    if (ier /= 0) then
...
!------------------------------------------------------------------------
    ...
    if (domain%regional) then
       ...
       if (domain%decomped) then
          call clmmpi_reduce(lsum,gsum,1,CLMMPI_REAL8,CLMMPI_SUM,masterproc, &
                          clmmpicom,ier)
       endif
