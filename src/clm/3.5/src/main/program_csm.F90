...
! !LOCAL VARIABLES:
  ...
  integer  :: clmmpicom_top   ! CLMMPI communicator
...
!-----------------------------------------------------------------------
  ...
  call csm_setup(clmmpicom_top)
  call spmd_init(clmmpicom_top)
  ...
  call t_initf(nlfilename, LogPrint=masterproc, CLMMpicom=clmmpicom_top, &
  ...
  call t_prf('timing_all',clmmpicom_top)
  call t_finalizef()
  call csm_shutdown()
