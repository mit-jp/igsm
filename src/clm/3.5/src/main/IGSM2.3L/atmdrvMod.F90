...
! !USES:
  ...
  use spmdMod      , only : masterproc, clmmpicom, comp_id, CLMMPI_REAL8, CLMMPI_INTEGER, iam
  use clm_mct_mod
...
#if (defined COUP_MIT2D | defined FORC_IGSM)
    j1 = 0
    !write (6,*) 'ATMDRV: datlat = ',datlat
    !write (6,*) 'ATMDRV: datlon = ',datlon
    do j = 1, datlat
       if ( j /= 9 .and. j/= 45 .and. j /= 46 ) then
        j1 = j1 + 1
        do i = 1, datlon
            n = (j1-1)*datlon + i
            aV_atm_d2a%rAttr(if_txy,n) = tslmit(1,j)
            ...
            aV_atm_d2a%rAttr(iprcxy,n) = pcpcmit(1,j)/3600.
            aV_atm_d2a%rAttr(iprlxy,n) = pcplmit(1,j)/3600.
#if (defined STOCHASTIC)
!CAS
...
#endif
#if (defined 2datm3dlnd)
            aV_atm_d2a%rAttr(if_txy,n) = cxysat(i,j)           
            aV_atm_d2a%rAttr(iprcxy,n) = cxyppc(i,j)            
	    aV_atm_d2a%rAttr(iprlxy,n) = pcplmit(1,j)/3600.
#endif
            aV_atm_d2a%rAttr(iflwdsxy,n) = dlwmit(1,j)
#if (defined COUP_MIT2D | defined FORC_IGSM | 2datm3dlnd)			
        enddo
       endif
     enddo
#else
...
!------------------------------------------------------------------------
...
#else
    filenam = trim(offline_atmdir) 
    write(6,*) 'ATMDRV_INIT: atm datafile - ',filenam
#endif
    ...
    pe_loc = root
    call mct_gsMap_init(gsMap_drv_glo0,ngseg,start,length,pe_loc,root,clmmpicom,comp_id)
    ...
    ns = mct_gsMap_lsize(gsMap_drv_glo0, clmmpicom)
    call mct_aVect_init(aV_drv_d2a,rlist=str,lsize=ns)
    ns = mct_gsMap_lsize(gsMap_atm_gdc2glo, clmmpicom)
...
!------------------------------------------------------------------------
...
#else
       locfn = trim(offline_atmdir) 
       write(6,*) 'ATM_OPENFILE: atm datafile - ',locfn
#endif
    ...
    call clmmpi_bcast (ntim, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
...
!                 write(6,*)'ATM warning: qsat < q!'
          ...
          if (nint(x(i,j,6)) == -1) then
             aV_drv_d2a%rAttr(izgcmxy,n) = 100._r8
          else
...
! !DESCRIPTION:
!
!	PROJECTION OF 2D IGSM ATMOSPHERIC VARIABLES ONTO THE 3D CLM GRID
!
!	NOTE:	THIS IS AN AD-HOC PROCEDURE TO ACCOMMODATE A 2X2.5 GRID
!
!	NOTE: 'latincr' is used to account for translation between 4 and 2 degree
!		   lat bands of the 2D IGSM and its 3D implementation
!
! this is done at every subsequent call to atmrd
!
! !USES:
...
!
! LOCAL VARIABLES:
    integer :: i,j,j1,k,n                  !do loop indices
    integer :: ier                      !error status
    integer :: varid                    !netCDF variable id
    integer :: status                   !netCDF error status
    integer :: beg3d(3)                 !netCDF 3-d start index (where to read first value)
    integer :: len3d(3)                 !netCDF 3-d count index (number of values to read)
    character(len=32) :: subname = 'readIGSM2'
!
! atm input field names
!
	include 'IGSM2.inc'
    integer,parameter:: fldsize=14
	real, parameter :: latincr=2.0	!NOTE: THIS SHOULD BE GENERALIZED, FOR NOW, 4 TO 2 DEGREE BANDS
    character(len=8) :: fldlst(fldsize)      !name of possible atm fields in input file
    data fldlst( 1) /'TBOT    '/
    data fldlst( 2) /'WIND    '/
    data fldlst( 3) /'QBOT    '/
...
!
	 ...
	 enddo
     !write (6,*) 'PROGRAM_OFF:'
     !write (6,*) 'Read and convert 2D IGSM to 3D CLM grid'
   endif
