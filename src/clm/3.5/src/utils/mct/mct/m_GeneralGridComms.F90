...
!                 error checks to avoid making bad CLMMPI calls
...
! transfer, up to five {\tt List\_send} operations (two {\tt CLMMPI\_SEND} 
...
! (which comprises eight {\tt CLMMPI\_SEND} operations).
!
! !INTERFACE:
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_SEND(HeaderAssoc, 6, MP_LOGICAL, dest, TagBase, ThisMCTWorld%MCT_comm, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: CLMMPI_SEND(HeaderAssoc...',ierr)
  endif
  ...
  if(HeaderAssoc(3)) then
     ...
     call CLMMPI_SEND(size(iGGrid%descend), 1, MP_type(size(iGGrid%descend)), &
                   dest, TagBase+5, ThisMCTWorld%MCT_comm, ierr)
     if(ierr /= 0) then
	call MP_perr_die(myname_,':: call CLMMPI_SEND(size(iGGrid%descend)...',ierr)
     endif

     call CLMMPI_SEND(iGGrid%descend, size(iGGrid%descend), MP_type(iGGrid%descend(1)), &
                   dest, TagBase+6, ThisMCTWorld%MCT_comm, ierr)
     if(ierr /= 0) then
	call MP_perr_die(myname_,':: call CLMMPI_SEND(iGGrid%descend...',ierr)
     endif
...
! transfer, up to five {\tt List\_recv} operations (two {\tt CLMMPI\_RECV} 
...
! (which comprises eight {\tt CLMMPI\_RECV} operations).
!
! !INTERFACE:
...
! for now, assume the components root is the source.
  ...
  call CLMMPI_RECV(HeaderAssoc, 6, MP_LOGICAL, source, TagBase, ThisMCTWorld%MCT_comm, MPstatus, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: CLMMPI_RECV(HeaderAssoc...',ierr)
  endif
  ...
  if(HeaderAssoc(3)) then
     ...
     call CLMMPI_RECV(DescendSize, 1, MP_type(DescendSize), &
                   source, TagBase+5, ThisMCTWorld%MCT_comm, MPstatus, ierr)
     if(ierr /= 0) then
	   call MP_perr_die(myname_,':: call CLMMPI_RECV(size(oGGrid%descend)...',ierr)
     endif
	...
	if(present(status)) then
     ...
     call CLMMPI_RECV(oGGrid%descend, DescendSize, MP_type(oGGrid%descend(1)), &
                   source, TagBase+6, ThisMCTWorld%MCT_comm, MPstatus, ierr)
     if(ierr /= 0) then
	   call MP_perr_die(myname_,':: call CLMMPI_RECV(oGGrid%descend...',ierr)
     endif
...
!                 to CLMMPI-style (i.e. iG valid on root only).
!       13Jun01 - J.W. Larson <larson@mcs.anl.gov> - Initialize stat
!                 (if present).
!EOP ___________________________________________________________________
...
! Header-Assocation array
  ...
  call CLMMPI_BCAST(HeaderAssoc,6,MP_LOGICAL,root,comm,ierr)
  ...
  if(HeaderAssoc(6)) then
     ...
     call CLMMPI_BCAST(DescendSize, 1, MP_INTEGER, root, comm, ierr)
     if(ierr /= 0) then
     ...
     call CLMMPI_BCAST(ioGGrid%descend, DescendSize, MP_LOGICAL, root, &
                    comm, ierr)
     if(ierr /= 0) then
