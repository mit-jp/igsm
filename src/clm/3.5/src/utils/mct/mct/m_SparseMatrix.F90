...
!
  ...
  call CLMMPI_ALLREDUCE(MyNumElements, GNumElements, 1, MP_INTEGER, &
                     MP_SUM, comm, ierr)
  if(ierr /= 0) then
...
!
  ...
  if(present(comm)) then
     call CLMMPI_ALLREDUCE(MySparsity, sparsity, 1, MP_INTEGER, &
                        MP_SUM, comm, ierr)
     if(ierr /= 0) then
...
! argument {\tt comm} is the Fortran 90 CLMMPI communicator handle used to
...
!           to set type in the clmmpi_allreduce
!EOP ___________________________________________________________________
!
  ...
  call CLMMPI_ALLREDUCE(end_row, num_rows, 1, MP_INTEGER, MP_MAX, &
                    comm, ierr)
  if(ierr /= 0) then
  ...
  call CLMMPI_ALLREDUCE(lsums, gsums, num_rows, mp_Type_lsums, MP_SUM, comm, ierr)
  if(ierr /= 0) then
