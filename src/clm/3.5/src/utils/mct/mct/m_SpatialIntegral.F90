...
! argument {\tt comm} (a Fortran CLMMPI communicator handle), the summation
...
! supplied (which corresponds to a Fortran CLMMPI communicatior handle).
!
! {\bf N.B.:  } The local lengths of the {\tt AttrVect} argument {\tt inAv} 
...
! Fortran CLMMPI communicatior handle).
!
! {\bf N.B.:  } The local lengths of the {\tt AttrVect} argument {\tt inAv} 
...
! to reduce communication latency costs), and the Fortran CLMMPI communicator
...
! costs), and the Fortran CLMMPI communicator handle is defined by the input 
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_AllReduce(PairedBuffer, OutPairedBuffer, PairedBufferLength, &
...
! is to reduce communication latency costs), and the Fortran CLMMPI communicator 
! handle is defined by the input {\tt INTEGER} argument {\tt comm}.  The 
! summation is an AllReduce operation, with all processes receiving the 
! global sum.
!
