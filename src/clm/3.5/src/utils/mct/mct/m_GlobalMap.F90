...
! one-dimensional domain decomposition for a vector on an CLMMPI communicator.  
...
! \item The segments are laid out in identical order to the CLMMPI rank of 
...
! this information with CLMMPI's scatter and gather operations).  We refer 
...
! whose indices run from zero to $N-1$, where $N$ is the number of CLMMPI 
! processes on the communicator on which the {\tt GlobalMap} is defined.
! This is done so this information corresponds directly to the CLMMPI process 
...
! data spread across the CLMMPI communicatior associated with the input 
...
! !INPUT PARAMETERS:
      ...
      integer,         intent(in)  :: comm    ! f90 CLMMPI communicator 
                                              ! handle 
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_allgather(ln,1,MP_INTEGER,GMap%counts,1,MP_INTEGER,comm,ier)
...
! it to all processes on the communicator associated with the CLMMPI 
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_bcast(GMap%counts, nPEs, MP_INTEGER, root, comm, ier)
  ...
  call CLMMPI_bcast(GMap%comp_id,1,MP_INTEGER,root,comm,ier)
  if(ier/=0) call MP_perr_die(myname_,'MPI_bcast()',ier)
...
! process in this operation has CLMMPI process ID defined by the input 
! {\tt INTEGER} argument {\tt my\_root}, and its CLMMPI communinicator 
! is defined by the input {\tt INTEGER} argument {\tt my\_comm}.  The 
! input {\tt INTEGER} argument {\tt remote\_npes} is the number of CLMMPI 
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_bcast(nPEs, 1, MP_INTEGER, my_root, my_comm, ier)
  ...
  call CLMMPI_bcast(GMap%counts, nPEs, MP_INTEGER, my_root, my_comm, ier)
  if(ier/=0) call MP_perr_die(myname_,'MPI_bcast()',ier)
  ...
  call CLMMPI_bcast(GMap%comp_id, 1, MP_INTEGER, my_root, my_comm,ier)
