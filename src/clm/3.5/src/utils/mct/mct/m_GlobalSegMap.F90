...
! the F90 {\tt INTEGER} handle for the CLMMPI communicator.  If the input
...
!                 the tag arguments to CLMMPI_IRECV; a bug in the declaration
!                 of the array status used by CLMMPI_WAITALL.
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_COMM_SIZE(my_comm, npes, ier)
  ...
  if(myID == root) then
     do i=0,npes-1
	call CLMMPI_IRECV(counts(i), 1, MP_INTEGER, i, i, &
	     my_comm, reqs(i), ier)
     end do
  endif

  call CLMMPI_SEND(ngseg, 1, MP_INTEGER, root, myID, my_comm, ier)
  if(ier /= 0) call MP_perr_die(myname_,'MPI_COMM_SIZE()',ier)

  if(myID == root) then
     call CLMMPI_WAITALL(size(reqs), reqs, status, ier)
     if(ier /= 0) call MP_perr_die(myname_,'MPI_WAITALL()',ier)
  endif

  call CLMMPI_BARRIER(my_comm, ier)
  if(ier /= 0) call MP_perr_die(myname_,'MPI_WAITALL()',ier)
  ...
  if(myID == root) then
	...
	call die(myname_, 'allocate((non)root_start...',ier)
        ...
        ! sends, followed by a call to CLMMPI_WAITALL().

  if(myID == root) then
     do i=0,npes-1
	call CLMMPI_IRECV(root_start(displs(i)), counts(i), MP_INTEGER, &
	     i, i, my_comm, reqs(i), ier)
     end do
  endif

  call CLMMPI_SEND(start, size(start), MP_INTEGER, root, myID, my_comm, ier)
  if(ier /= 0) call MP_perr_die(myname_,'MPI_COMM_SIZE()',ier)
  ...
  if(myID == root) then
     do i=0,npes-1
	call CLMMPI_IRECV(root_length(displs(i)), counts(i), MP_INTEGER, &
	     i, i, my_comm, reqs(i), ier)
     end do
  endif

  call CLMMPI_SEND(length, size(length), MP_INTEGER, root, myID, my_comm, ier)
  if(ier /= 0) call MP_perr_die(myname_,'MPI_COMM_SIZE()',ier)
  ...
  if(myID == root) then
     do i=0,npes-1
	call CLMMPI_IRECV(root_pe_loc(displs(i)), counts(i), MP_INTEGER, &
	     i, i, my_comm, reqs(i), ier)
     end do
  endif

  call CLMMPI_SEND(my_pe_loc, size(my_pe_loc), MP_INTEGER, root, myID, &
       my_comm, ier)
  if(ier /= 0) call MP_perr_die(myname_,'MPI_COMM_SIZE()',ier)
...
! {\tt my\_comm} is the F90 {\tt INTEGER} handle for the CLMMPI communicator.
!
! !INTERFACE:
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_COMM_RANK(my_comm, myID, ier)
  ...
  call CLMMPI_BCAST(GSMap%ngseg, 1, MP_INTEGER, root, my_comm, ier)
  if(ier/=0) call MP_perr_die(myname_,'MPI_BCAST(GSmap%ngseg)',ier)

        ! Broadcast the value of GSMap%comp_id

  call CLMMPI_BCAST(GSMap%comp_id, 1, MP_INTEGER, root, my_comm, ier)
  ...
  call CLMMPI_BCAST(GSMap%start, GSMap%ngseg, MP_INTEGER, root, my_comm, ier)
  if(ier/=0) call MP_perr_die(myname_,'MPI_BCAST(GSMap%start)',ier)

  call CLMMPI_BCAST(GSMap%length, GSMap%ngseg, MP_INTEGER, root, my_comm, ier)
  if(ier/=0) call MP_perr_die(myname_,'MPI_BCAST(GSMap%length)',ier)

  call CLMMPI_BCAST(GSMap%pe_loc, GSMap%ngseg, MP_INTEGER, root, my_comm, ier)
  ...
  if(present(gsize)) then
	...
	GSMap%gsize = gsize
     ...
     call CLMMPI_BCAST(GSMap%gsize, 1, MP_INTEGER, root, my_comm, ier)
...
! !INPUT PARAMETERS: 
     ...
     integer , intent(in) :: my_comm         ! clmmpi communicator group (mine)
...
!EOP ___________________________________________________________________
     ...
     integer,parameter   :: pid0=0       ! clmmpi process id for root pe
...
!! 
   if (mysize==0) then
     ...
     call CLMMPI_COMM_RANK(my_comm,rank, ierr)
...
! argument {\tt comm}, which is the Fortran handle for the CLMMPI 
! communicator.
!
! !INTERFACE:
