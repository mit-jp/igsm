...
! which encompasses all the models (typically this can be CLMMPI\_COMM\_WORLD).
...
!  First on the global root, post a receive for each component
  if(myGid == 0) then
    do i=1,ncomps
       call CLMMPI_IRECV(root_nprocs(i), 1, MP_INTEGER, MP_ANY_SOURCE,i, &
...
!  The local root on each component sends
  if(myLid == 0) then
    if(present(myids)) then
      do i=1,size(myids)
        call CLMMPI_SEND(mysize,1,MP_INTEGER,0,myids(i),globalcomm,ier)
        if(ier /= 0) call MP_perr_die(myname_,'MPI_SEND(mysize)',ier)
      enddo
    else
        call CLMMPI_SEND(mysize,1,MP_INTEGER,0,myid,globalcomm,ier)
        if(ier /= 0) call MP_perr_die(myname_,'MPI_SEND(mysize)',ier)
    endif
  endif
...
! gather over the LOCAL comm
  call CLMMPI_GATHER(myGid,1,MP_INTEGER,Gprocids,1,MP_INTEGER,0,mycomm,ier)
...
!  First on the global root, post a receive for each component
  if(myGid == 0) then
    do i=1,ncomps
       ...
       call CLMMPI_IRECV(apoint(1), root_nprocs(i),MP_INTEGER, &
...
!  The root on each component sends
  if(myLid == 0) then
    if(present(myids)) then
      do i=1,size(myids)
        call CLMMPI_SEND(Gprocids,mysize,MP_INTEGER,0,myids(i),globalcomm,ier)
        if(ier /= 0) call MP_perr_die(myname_,'MPI_SEND(Gprocids)',ier)
      enddo
    else
        call CLMMPI_SEND(Gprocids,mysize,MP_INTEGER,0,myid,globalcomm,ier)
        if(ier /= 0) call MP_perr_die(myname_,'MPI_SEND(Gprocids)',ier)
    endif
  endif
...
! Now store the component ids in the World description and Broadcast
  ...
  call CLMMPI_BCAST(ThisMCTWorld%nprocspid, ncomps, MP_INTEGER, 0, MCTcomm, ier)
  if(ier/=0) call MP_perr_die(myname_,'MPI_BCast nprocspid',ier)

  call CLMMPI_BCAST(ThisMCTWorld%idGprocid, ncomps*Gsize,MP_INTEGER, 0,MCTcomm, ier)
