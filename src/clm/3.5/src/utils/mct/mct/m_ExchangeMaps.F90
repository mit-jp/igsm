...
!EOP ___________________________________________________________________
  ...
  if(myID == 0) then ! I am the root on LocalComm
     ...
     call CLMMPI_SEND(LocalMapPars, NumHandshakePars, MP_INTEGER, &
	           RemoteRootID, SendTag, ThisMCTWorld%MCT_comm, ierr)
     if(ierr /= 0) call MP_perr_die(myname_,'call CLMMPI_SEND()',ierr)

      ! Post receive from RemoteRootID:

     call CLMMPI_RECV(RemoteMapPars, NumHandshakePars, MP_INTEGER, &
	           RemoteRootID, RecvTag, ThisMCTWorld%MCT_comm, status, ierr)
     if(ierr /= 0) call MP_perr_die(myname_,'call CLMMPI_RECV()',ierr)
...
!           RecvTag values
!  3May01 - R.L. Jacob <jacob@mcs.anl.gov> - change CLMMPI_SEND to
!           CLMMPI_ISEND to avoid possible buffering problems seen
!           on IBM SP.
!EOP ___________________________________________________________________
...
! Storage arrays for Remote GlobalSegMap data:
  ...
  if(myID == root) then
	...
	call die(myname_,'Component ID mismatch',ierr)
     ...
     call CLMMPI_ISEND(SendBuf(1), 3*LocalMapPars(NumSegIndex), MP_INTEGER, &
