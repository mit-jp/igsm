...
!	28Sep99 - Jing Guo	- Merged with the CLMMPI version with
...
! !INTERFACE:
    ...
    subroutine I90_allLoadF(fname,root,comm,istat)
      use m_clmmpif90, only : MP_perr
      use m_clmmpif90, only : MP_comm_rank
      use m_clmmpif90, only : MP_CHARACTER
      use m_clmmpif90, only : MP_INTEGER
      use m_die, only : perr
      implicit none
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_Bcast(i90_now%buffer,NBUF_MAX,MP_CHARACTER,root,comm,ier)
  if(ier /= 0) then
  ...
  call CLMMPI_Bcast(i90_now%nbuf,1,MP_INTEGER,root,comm,ier)
  if(ier /= 0) then
