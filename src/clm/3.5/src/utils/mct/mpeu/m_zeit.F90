...
! !INTERFACE:
	...
	! Flags of all printable timers
      ...
      public ::  MWTIME	! CLMMPI_Wtime() wall-clock time
...
!		. rewritten for possible CLMMPI applications, with
...
!		  timing functions.
!		. used CLMMPI_Wtime() for the CLMMPI standard conforming
...
! !INTERFACE:
    ...
    subroutine allflush_(comm,root,lu,umask)
      ...
      use m_ioutil, only : luflush
      use m_die, only : die
      use m_clmmpif90,only : MP_wtime,MP_type
      use m_clmmpif90,only : MP_comm_size,MP_comm_rank
...
!_______________________________________________________________________
  ...
  if(myID /= root) then
    ...
    call CLMMPI_gather((ztbf(0:5,0:1,indx_(0:mnm))),nbf,mp_Type_ztbf, &
		    ztbf,nbf,mp_Type_ztbf,root,comm,ier	)
    if(ier /= 0) then
      write(stderr,'(2a,i3)') myname_,	&
	': CLMMPI_gather(!root) error, ier =',ier
      call die(myname_)
    endif
