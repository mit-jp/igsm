...
!      
! !INTERFACE: ------------------------------------------------------------------
SUBROUTINE eshr_estate_getStats( eState, clmmpicom, rootid )
    use shr_kind_mod,    only: SHR_KIND_CL
    use shr_const_mod,   only: SHR_CONST_SPVAL
    use shr_clmmpi_mod,     only: shr_clmmpi_gatherv, shr_clmmpi_gathScatVInit, &
                               shr_clmmpi_commrank, shr_clmmpi_barrier
    implicit none

! !INPUT/OUTPUT PARAMETERS:
    type(ESMF_State),  intent(IN) :: EState      ! Input state to get the stats on
    integer, optional, intent(IN) :: clmmpicom      ! CLMMpi communicator to gather to
...
!EOP
    ...
    integer                             :: clmmpiRank       ! CLMMPI rank
    character(len=SHR_KIND_CL), pointer :: FieldNames(:) ! Names of fields  in state
...
!-------------------------------------------------------------------------------
   ...
   if ( present(clmmpicom) .and. present(rootid) )then
      call shr_clmmpi_commrank( clmmpicom, clmmpiRank )
      if ( clmmpiRank /= rootid )then
         printIt = .false.
      else
         printIt = .true.
      end if
   ...
   do j = 1, nFields
      ...
      if ( (dataType == ESMF_DATA_REAL) .and. (kind == ESMF_R8) .and. (rank == 2) )then
         ...
         call eshr_rc_check( rc, subName//': Error in getting F90 ptr to field data' )
         if ( present(clmmpicom) .and. present(rootid) )then
            call shr_clmmpi_gathScatvInit( clmmpicom, rootid, edata1D, data1D, globSize, &
                                        displs )
            call shr_clmmpi_barrier( clmmpicom )
            call shr_clmmpi_gatherv( edata1D, size(edata1D), data1D, globSize, displs,  &
                                  rootid, clmmpicom )
            call shr_clmmpi_barrier( clmmpicom )
         else
            data1D => edata1D
