...
! call eshr_timemgr_NMLinfoRead( nlfilename, LogPrint, CLMMPICom, MasterTask, &
!                                clockNMLinfo )
...
!     call eshr_timemgr_clockRestWrite( restart_file, CLMMPICom, MasterTask, clock )
...
! call eshr_timemgr_NMLinfo( restart_file, clock, LogPrint, CLMMPICom, &
...
SUBROUTINE eshr_timemgr_NMLinfoRead( nlfilename, LogPrint, CLMMPICom, MasterTask, &
                                     clockNMLinfoOut )
! !USES:
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   logical, optional,           intent(IN)    :: MasterTask  ! If root CLMMPI task or not
...
!EOP
    ...
    logical                 :: MasterTask2           ! If root CLMMPI task or not
...
!-------------------------------------------------------------------------------
    ...
    !---------------------------------------------------------------------------
    ! If CLMMPI broadcast values to all tasks
    !---------------------------------------------------------------------------
    if ( present(CLMMPICOM) )then
       call shr_clmmpi_bcast( restart_n,                CLMMPICom )
       call shr_clmmpi_bcast( restart_option,           CLMMPICom )
       call shr_clmmpi_bcast( stop_n,                   CLMMPICom )
       call shr_clmmpi_bcast( stop_option,              CLMMPICom )
       call shr_clmmpi_bcast( stop_ymd,                 CLMMPICom )
       call shr_clmmpi_bcast( stop_tod,                 CLMMPICom )
       call shr_clmmpi_bcast( stop_final_ymd,           CLMMPICom )
       call shr_clmmpi_bcast( atm_cpl_dt,               CLMMPICom )
       call shr_clmmpi_bcast( lnd_cpl_dt,               CLMMPICom )
       call shr_clmmpi_bcast( ice_cpl_dt,               CLMMPICom )
       call shr_clmmpi_bcast( ocn_cpl_dt,               CLMMPICom )
       call shr_clmmpi_bcast( perpetual_ymd,            CLMMPICom )
       call shr_clmmpi_bcast( start_ymd,                CLMMPICom )
       call shr_clmmpi_bcast( start_tod,                CLMMPICom )
       call shr_clmmpi_bcast( ref_ymd,                  CLMMPICom )
       call shr_clmmpi_bcast( ref_tod,                  CLMMPICom )
       call shr_clmmpi_bcast( perpetual_run,            CLMMPICom )
       call shr_clmmpi_bcast( orb_iyear_AD,             CLMMPICom )
       call shr_clmmpi_bcast( orb_obliq,                CLMMPICom )
       call shr_clmmpi_bcast( orb_eccen,                CLMMPICom )
       call shr_clmmpi_bcast( orb_mvelp,                CLMMPICom )
       call shr_clmmpi_bcast( calendar,                 CLMMPICom )
       call shr_clmmpi_bcast( restart_file_TMGoverRide, CLMMPICom )
    end if
...
SUBROUTINE eshr_timemgr_NMLinfoRestRead( restart_file, CLMMPICom, &
                                         MasterTask, clockNMLinfo )
! !USES:
...
!-------------------------------------------------------------------------------
   ...
   if ( present(CLMMPICom) )then
      call shr_ncio_descripRead( ncId, NClockVars, prefix//"clock_", CLMMPICom, &
   ...
   !----------------------------------------------------------------------------
   if ( present(CLMMPICom) )then
      call eshr_timemgr_clockInfoRestRead( restart_file, CLMMPICom,     &
                                           MasterTask2,              &
...
SUBROUTINE eshr_timemgr_clockRestWrite( restart_file, CLMMPICom, MasterTask, clock )
...
!-------------------------------------------------------------------------------
  ...
  if ( present(CLMMPICom) )then
     call shr_ncio_descripWrite( ncId, NClockVars, prefix//"clock_", clmmpicom, &
  ...
  if ( present(CLMMPICom) )then
     call eshr_timemgr_clockInfoRestWrit( restart_file, CLMMPICom, MasterTask2, &
                                          clockInfo=clock%info )
  else
...
SUBROUTINE eshr_timemgr_clockInfoRestRead( restart_file, CLMMPICom,   &
...
!-------------------------------------------------------------------------------
   ...
   if ( present(CLMMPICom) )then
      call shr_ncio_descripRead( ncId, NInfoVars,  prefix//"ClockInfo_",     &
                                 CLMMPICom, MasterTask2, var=ClockInfo%var )
   else
      call shr_ncio_descripRead( ncId, NInfoVars,  prefix//"ClockInfo_",     &
                                 var=ClockInfo%var )
   end if
...
SUBROUTINE eshr_timemgr_clockInfoRestWrit( restart_file, CLMMPICom, MasterTask, &
                                           clockInfo )
! !USES:
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   integer, optional,                intent(IN)    :: CLMMPICom       ! CLMMPI communicator
   logical, optional,                intent(IN)    :: MasterTask   ! If root CLMMPI task
...
!-------------------------------------------------------------------------------
  ...
  if ( present(CLMMPICom) )then
     call shr_ncio_descripWrite( ncId, NInfoVars,  prefix//"ClockInfo_", &
                                 clmmpicom, MasterTask2, exists=exists,     &
                                 var=clockInfo%var )
  else
     call shr_ncio_descripWrite( ncId, NInfoVars,  prefix//"ClockInfo_", &
