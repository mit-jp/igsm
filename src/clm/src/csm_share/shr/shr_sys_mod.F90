...
MODULE shr_sys_mod
   ...
   use shr_clmmpi_mod   ! wraps CLMMPI layer
...
!-------------------------------------------------------------------------------
   ...
   write(6,F00) 'WARNING: calling shr_clmmpi_abort() and stopping'
   call shr_sys_flush(6)
   call shr_clmmpi_initialized(flag)
   if (flag) then
     if (present(string).and.present(rc)) then
       call shr_clmmpi_abort(trim(string),rc)
     elseif (present(string)) then
       call shr_clmmpi_abort(trim(string))
     elseif (present(rc)) then
       call shr_clmmpi_abort(rcode=rc)
     else
       call shr_clmmpi_abort()
     endif
   endif
   call shr_sys_flush(6)
   call abort()
