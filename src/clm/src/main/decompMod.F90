...
! !USES:
  ...
  use spmdMod     , only : masterproc, iam, npes, clmmpicom, comp_id
  use clm_mct_mod
...
! !PUBLIC TYPES:
  implicit none
  integer, public :: clump_pproc ! number of clumps per CLMMPI process
!
...
!    if (masterproc) write(6,*) 'precompute total anumg ',anumg_tot,anumpc,anumxtra
    ...
    call mct_gsMap_init(gsMap_lnd_gdc2glo, gindex, clmmpicom, comp_id, lsize, gsize )
    deallocate(gindex)
    ...
    call mct_gsMap_init(gsMap_atm_gdc2glo, gindex, clmmpicom, comp_id, lsize, gsize )
    deallocate(gindex)
...
!             ln = lnmap(lnoff(an)+lnidx)
          ...
          enddo
    enddo
    call clmmpi_allreduce(allvecl,allvecg,size(allvecg),CLMMPI_INTEGER,CLMMPI_SUM,clmmpicom,ier)
