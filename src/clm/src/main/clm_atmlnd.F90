...
type atm2lnd_type
  ...
  real(r8), pointer :: forc_strm_dur(:)!Storm Duration
  real(r8), pointer :: forc_strm_dry(:)!Inter-storm period
  real(r8), pointer :: forc_strms(:)   !# of storms
#endif
...
#if (defined STOCHASTIC)
  allocate(a2l%forc_strm_dur(beg:end))
  allocate(a2l%forc_strm_dry(beg:end))
  allocate(a2l%forc_strms(beg:end))
#endif
...
#if (defined STOCHASTIC)
  a2l%forc_strm_dur = ival
  a2l%forc_strm_dry = ival
  a2l%forc_strms = ival
#endif
...
!------------------------------------------------------------------------
  ...
  allocate(l2a%qflx_drai(beg:end))
  allocate(l2a%qflx_qrgwl(beg:end))
  allocate(l2a%qflx_tran_veg(beg:end))
  allocate(l2a%qflx_evap_can(beg:end))
  allocate(l2a%qflx_evap_soil(beg:end))
  allocate(l2a%t_grnd(beg:end))
  allocate(l2a%frac_sno(beg:end))
  allocate(l2a%h2ol(beg:end,1:nlevsoi))
  allocate(l2a%h2oi(beg:end,1:nlevsoi))
  allocate(l2a%tsoi(beg:end,1:nlevsoi))
#endif
...
! ival = nan      ! causes core dump in map_maparray, tcx fix
  ...
  l2a%qflx_drai(beg:end) = ival
  l2a%qflx_qrgwl(beg:end) = ival
  l2a%qflx_tran_veg(beg:end) = ival
  l2a%qflx_evap_can(beg:end) = ival
  l2a%qflx_evap_soil(beg:end) = ival
  l2a%t_grnd(beg:end) = ival
  l2a%frac_sno(beg:end) = ival
  l2a%h2ol(beg:end,1:nlevsoi) = ival
  l2a%h2oi(beg:end,1:nlevsoi) = ival
  l2a%tsoi(beg:end,1:nlevsoi) = ival
#endif
...
#if (defined STOCHASTIC)
  ix=ix+1; asrc(:,ix) = a2l_src%forc_strm_dur(:)  
  ix=ix+1; asrc(:,ix) = a2l_src%forc_strm_dry(:)  
  ix=ix+1; asrc(:,ix) = a2l_src%forc_strms(:)  
#endif
  do n = 1,numrad
...
#if (defined STOCHASTIC)
  ix=ix+1; a2l_src%forc_strm_dur(:) = adst(:,ix)
  ix=ix+1; a2l_src%forc_strm_dry(:) = adst(:,ix)
  ix=ix+1; a2l_src%forc_strms(:)    = adst(:,ix)
#endif
  do n = 1,numrad
...
!cdir nodep
     ...
     call p2g(begp, endp, begc, endc, begl, endl, begg, endg, &
          pptr%pwf%qflx_evap_can, clm_l2a%qflx_evap_can, &
          p2c_scale_type='unity', c2l_scale_type= 'unity', l2g_scale_type='unity')

     call p2g(begp, endp, begc, endc, begl, endl, begg, endg, &
          pptr%pwf%qflx_tran_veg, clm_l2a%qflx_tran_veg, &
          p2c_scale_type='unity', c2l_scale_type= 'unity', l2g_scale_type='unity')

     call p2g(begp, endp, begc, endc, begl, endl, begg, endg, &
          pptr%pwf%qflx_evap_soi, clm_l2a%qflx_evap_soil, &
          p2c_scale_type='unity', c2l_scale_type= 'unity', l2g_scale_type='unity')

     call c2g(begc, endc, begl, endl, begg, endg, cptr%cwf%qflx_surf, clm_l2a%qflx_surf,&
          c2l_scale_type= 'unity', l2g_scale_type='unity')

     call c2g(begc, endc, begl, endl, begg, endg, cptr%cwf%qflx_drain, clm_l2a%qflx_drai,&
          c2l_scale_type= 'unity', l2g_scale_type='unity')

     call c2g(begc, endc, begl, endl, begg, endg, cptr%cwf%qflx_qrgwl, clm_l2a%qflx_qrgwl,&
          c2l_scale_type= 'unity', l2g_scale_type='unity')

     call c2g(begc, endc, begl, endl, begg, endg, cptr%cps%frac_sno, clm_l2a%frac_sno,&
          c2l_scale_type= 'unity', l2g_scale_type='unity')

     call c2g(begc, endc, begl, endl, begg, endg, cptr%ces%t_grnd, clm_l2a%t_grnd,&
          c2l_scale_type= 'unity', l2g_scale_type='unity')

     do g = begg,endg
       g2i = g
!CAS
!CAS    Account for offset required for latitudes north of Antarctica
!CAS
       if ( g >= 8 ) g2i = g2i + 2

       lhfclm(1,g2i) = -clm_l2a%eflx_lh_tot(g)
       shfclm(1,g2i) = -clm_l2a%eflx_sh_tot(g)
       tauxclm(1,g2i) = -clm_l2a%taux(g)
       tauyclm(1,g2i) = -clm_l2a%tauy(g)
       emvclm(1,g2i) = clm_l2a%qflx_evap_tot(g)
       sroclm(1,g2i) = clm_l2a%qflx_surf(g)
       ssrclm(1,g2i) = clm_l2a%qflx_drai(g)
       glrclm(1,g2i) = clm_l2a%qflx_qrgwl(g)
       vetclm(1,g2i) = clm_l2a%qflx_tran_veg(g)
       cevclm(1,g2i) = clm_l2a%qflx_evap_can(g)
       sevclm(1,g2i) = clm_l2a%qflx_evap_soil(g)
       lwuclm(1,g2i) = -clm_l2a%eflx_lwrad_out(g)
       tref2mclm(1,g2i) = clm_l2a%t_ref2m(g)
       tflxclm(1,g2i) = clm_l2a%t_rad(g)
       tgndclm(1,g2i) = clm_l2a%t_grnd(g)
       snwdclm(1,g2i) = clm_l2a%h2osno(g)
       snwcclm(1,g2i) = clm_l2a%frac_sno(g)
       do l = 1,nlevsoi
         call c2g(begc, endc, begl, endl, begg, endg, &
          cptr%ces%t_soisno(:,l), clm_l2a%tsoi(:,l), &
          c2l_scale_type= 'unity', l2g_scale_type='unity')

         call c2g(begc, endc, begl, endl, begg, endg, &
          cptr%cws%h2osoi_liq(:,l), clm_l2a%h2ol(:,l), &
          c2l_scale_type= 'unity', l2g_scale_type='unity')

         call c2g(begc, endc, begl, endl, begg, endg, &
          cptr%cws%h2osoi_ice(:,l), clm_l2a%h2oi(:,l), &
          c2l_scale_type= 'unity', l2g_scale_type='unity')

          h2olclm(1,g2i,l) = clm_l2a%h2ol(g,l)
          h2oiclm(1,g2i,l) = clm_l2a%h2oi(g,l)
          tsoiclm(1,g2i,l) = clm_l2a%tsoi(g,l)
       enddo
       asdirclm(1,g2i) = clm_l2a%albd(g,1)
       aldirclm(1,g2i) = clm_l2a%albd(g,2)
     ...
     !do l = 1,nlevsoi
       ...
       !write (6,*) h2olclm(1,:,l)
       !write (6,*) 'CLM_ATMLAND: FRZ. SOIL H2O TO IGSM'
       !write (6,*) h2oiclm(1,:,l)
       !write (6,*) 'CLM_ATMLAND: SOIL TEMP H2O TO IGSM'
       !write (6,*) tsoiclm(1,:,l)
     !enddo
!
!	Confirm data is being written to IGSM common block
!
         !write (6,*) 'CLM_ATMLAND: LATENT HEAT FLUX TO IGSM'
         !write (6,*) lhfclm
         !write (6,*) 'CLM_ATMLAND: SENSIBLE HEAT FLUX TO IGSM'
         !write (6,*) shfclm
         !write (6,*) 'CLM_ATMLAND: TAUX TO IGSM'
         !write (6,*) tauxclm
         !write (6,*) 'CLM_ATMLAND: TAUY TO IGSM'
         !write (6,*) tauyclm
         !write (6,*) 'CLM_ATMLAND: ABS. SW DIR TO IGSM'
         !write (6,*) asdirclm
         !write (6,*) 'CLM_ATMLAND: ABS. LW DIR TO IGSM'
         !write (6,*) aldirclm
         !write (6,*) 'CLM_ATMLAND: ABS. SW DIF TO IGSM'
         !write (6,*) asdifclm
         !write (6,*) 'CLM_ATMLAND: ABS. LW DIF TO IGSM'
         !write (6,*) aldifclm
         !write (6,*) 'CLM_ATMLAND: EVAPORATION TO IGSM'
         !write (6,*) emvclm
         !write (6,*) 'CLM_ATMLAND: LWUN TO IGSM'
         !write (6,*) lwuclm
         !write (6,*) 'CLM_ATMLAND: TREF TO IGSM'
         !write (6,*) tref2mclm
#endif
