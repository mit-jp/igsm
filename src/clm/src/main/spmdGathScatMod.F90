...
!BOP
!
! !IROUTINE: spmd_compute_clmmpigs
!
! !INTERFACE:
  subroutine spmd_compute_clmmpigs (clmlevel, nfact, numtot, numperproc, &
                                 displs, indexi)
!
! !DESCRIPTION:
...
!----------------------------------------------------------------------
    ...
    case default
       ...
       write(6,*) 'COMPUTE_CLMMPIGS: Invalid expansion character: ',trim(clmlevel)
       call endrun
  ...
  end subroutine spmd_compute_clmmpigs
...
!-----------------------------------------------------------------------
  ...
  call mct_aVect_scatter(AVi, AVo, gsmap, 0, clmmpicom)
...
!-----------------------------------------------------------------------
    call spmd_compute_clmmpigs (clmlevel, 1, numrecv, numsendv, displsv, beg)
    if (masterproc) then
       call clmmpi_scatterv (iglobal, numsendv, displsv, CLMMPI_INTEGER, &
            ilocal(beg), numrecv , CLMMPI_INTEGER , 0, clmmpicom, ier)
    else
       call clmmpi_scatterv (0, numsendv, displsv, CLMMPI_INTEGER, &
            ilocal(beg), numrecv , CLMMPI_INTEGER , 0, clmmpicom, ier)
    endif
    if (ier /= 0) then
...
!-----------------------------------------------------------------------
    call spmd_compute_clmmpigs (clmlevel, 1, numrecv, numsendv, displsv, beg)
    if (masterproc) then
       call clmmpi_scatterv (rglobal, numsendv, displsv, CLMMPI_REAL8, &
            rlocal(beg), numrecv , CLMMPI_REAL8 , 0, clmmpicom, ier)
    else
       call clmmpi_scatterv (0._r8, numsendv, displsv, CLMMPI_REAL8, &
            rlocal(beg), numrecv , CLMMPI_REAL8 , 0, clmmpicom, ier)
    endif
    if (ier/=0 ) then
...
!-----------------------------------------------------------------------
    ...
    call spmd_compute_clmmpigs (clmlevel, ndim1, numrecv, numsendv, displsv, beg)
    if (masterproc) then
       call clmmpi_scatterv (iglobal(l1,1), numsendv, displsv, CLMMPI_INTEGER, &
            ilocal(l1,beg), numrecv ,  CLMMPI_INTEGER, 0, clmmpicom, ier)
    else
       call clmmpi_scatterv (0, numsendv, displsv, CLMMPI_INTEGER, &
            ilocal(l1,beg), numrecv ,  CLMMPI_INTEGER, 0, clmmpicom, ier)
    endif
...
!-----------------------------------------------------------------------
    ...
    call spmd_compute_clmmpigs (clmlevel, ndim1, numrecv, numsendv, displsv, beg)
    if (masterproc) then
       call clmmpi_scatterv (rglobal(l1,1), numsendv, displsv, CLMMPI_REAL8, &
            rlocal(l1,beg), numrecv ,  CLMMPI_REAL8, 0, clmmpicom, ier)
    else
       call clmmpi_scatterv (0._r8, numsendv, displsv, CLMMPI_REAL8, &
            rlocal(l1,beg), numrecv ,  CLMMPI_REAL8, 0, clmmpicom, ier)
    endif
...
!-----------------------------------------------------------------------
  ...
  call mct_aVect_gather(AVi, AVo, gsmap, 0, clmmpicom)
...
!-----------------------------------------------------------------------
    call spmd_compute_clmmpigs (clmlevel, 1, numsend, numrecvv, displsv, beg)
    if (masterproc) then
       call clmmpi_gatherv (ilocal(beg), numsend , CLMMPI_INTEGER, &
            iglobal, numrecvv, displsv, CLMMPI_INTEGER, 0, clmmpicom, ier)
    else
       call clmmpi_gatherv (ilocal(beg), numsend , CLMMPI_INTEGER, &
            0, numrecvv, displsv, CLMMPI_INTEGER, 0, clmmpicom, ier)
    endif
    if (ier/=0 ) then
...
!-----------------------------------------------------------------------
    call spmd_compute_clmmpigs (clmlevel, 1, numsend, numrecvv, displsv, beg)
    if (masterproc) then
       call clmmpi_gatherv (rlocal(beg), numsend , CLMMPI_REAL8, &
            rglobal, numrecvv, displsv, CLMMPI_REAL8, 0, clmmpicom, ier)
    else
       call clmmpi_gatherv (rlocal(beg), numsend , CLMMPI_REAL8, &
            0._r8, numrecvv, displsv, CLMMPI_REAL8, 0, clmmpicom, ier)
    endif
    if (ier /= 0) then
...
!-----------------------------------------------------------------------
    ...
    l1 = lbound(ilocal,dim=1)
    ndim1 = size(ilocal,dim=1)
    call spmd_compute_clmmpigs (clmlevel, ndim1, numsend, numrecvv, displsv, beg)
    if (masterproc) then
       call clmmpi_gatherv (ilocal(l1,beg), numsend , CLMMPI_INTEGER, &
            iglobal(l1,1), numrecvv, displsv, CLMMPI_INTEGER, 0, clmmpicom, ier)
    else
       call clmmpi_gatherv (ilocal(l1,beg), numsend , CLMMPI_INTEGER, &
            0, numrecvv, displsv, CLMMPI_INTEGER, 0, clmmpicom, ier)
    endif
...
!-----------------------------------------------------------------------
    ...
    call spmd_compute_clmmpigs (clmlevel, ndim1, numsend, numrecvv, displsv, beg)
    if (masterproc) then
       call clmmpi_gatherv (rlocal(l1,beg), numsend , CLMMPI_REAL8, &
            rglobal(l1,1), numrecvv, displsv, CLMMPI_REAL8, 0, clmmpicom, ier)
    else
       call clmmpi_gatherv (rlocal(l1,beg), numsend , CLMMPI_REAL8, &
            0._r8, numrecvv, displsv, CLMMPI_REAL8, 0, clmmpicom, ier)
    endif
...
!-----------------------------------------------------------------------
    call spmd_compute_clmmpigs (clmlevel, 1, numsend, numrecvv, displsv, beg)
    call clmmpi_allgatherv (ilocal(beg), numsend , CLMMPI_INTEGER, &
                         iglobal, numrecvv, displsv, CLMMPI_INTEGER, clmmpicom, ier)
    if (ier/=0 ) then
       write(6,*)'gather_1darray_int error: ',ier
       call endrun
    endif
...
!-----------------------------------------------------------------------
    call spmd_compute_clmmpigs (clmlevel, 1, numsend, numrecvv, displsv, beg)
    call clmmpi_allgatherv (rlocal(beg), numsend , CLMMPI_REAL8, &
                         rglobal, numrecvv, displsv, CLMMPI_REAL8, clmmpicom, ier)
    if (ier /= 0) then
       write(6,*)'gather_1darray_real error: ',ier
       call endrun
    endif
...
!-----------------------------------------------------------------------
    ...
    ndim1 = size(ilocal,dim=1)
    call spmd_compute_clmmpigs (clmlevel, ndim1, numsend, numrecvv, displsv, beg)
    call clmmpi_allgatherv (ilocal(l1,beg), numsend , CLMMPI_INTEGER, &
                         iglobal(l1,1), numrecvv, displsv, CLMMPI_INTEGER, clmmpicom, ier)
    if (ier /= 0) then
       write(6,*)'gather_2darray_int error: ',ier
       call endrun
    endif
...
!-----------------------------------------------------------------------
    ...
    ndim1 = size(rlocal,dim=1)
    call spmd_compute_clmmpigs (clmlevel, ndim1, numsend, numrecvv, displsv, beg)
    call clmmpi_allgatherv (rlocal(l1,beg), numsend , CLMMPI_REAL8, &
                         rglobal(l1,1), numrecvv, displsv, CLMMPI_REAL8, clmmpicom, ier)
    if (ier /= 0) then
       write(6,*)'gather_2darray_real error: ',ier
       call endrun
    endif
