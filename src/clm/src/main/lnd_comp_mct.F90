...
!-----------------------------------------------------------------------
    ...
    ! set cdata data
    call seq_cdata_setptrs(cdata_l, ID=LNDID, clmmpicom=clmmpicom_lnd, &
         gsMap=GSMap_lnd, dom=dom_l, CCSMInit=CCSMInit)

    ! Initialize clm CLMMPI communicator 

    call spmd_init( clmmpicom_lnd )
    ...
    call lnd_SetgsMap_mct( clmmpicom_lnd, LNDID, gsMap_lnd ) 	
    lsize = mct_gsMap_lsize(gsMap_lnd, clmmpicom_lnd)
...
! !LOCAL VARIABLES:
    ...
    integer :: ier     ! CLMMPI error return
...
!=================================================================================
  ...
  subroutine lnd_SetgsMap_mct( clmmpicom_lnd, LNDID, gsMap_lnd )
    ...
    ! Arguments
    !
    integer        , intent(in)  :: clmmpicom_lnd
    ...
    call mct_gsMap_init( gsMap_lnd, gindex, clmmpicom_lnd, LNDID, lsize, gsize )
