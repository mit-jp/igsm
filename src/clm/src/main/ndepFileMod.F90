...
!cdir nodep
    ...
    call clmmpi_bcast (nt1   , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (nt2   , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (ntimes, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
    if (.not. masterproc) then
       allocate(yearsndep(ntimes))
    end if
    call clmmpi_bcast (yearsndep, size(yearsndep), CLMMPI_INTEGER, 0, clmmpicom, ier)
    call clmmpi_bcast (ndepdyn1  , size(ndepdyn1)  , CLMMPI_REAL8, 0, clmmpicom, ier)
    call clmmpi_bcast (ndepdyn2  , size(ndepdyn2)  , CLMMPI_REAL8, 0, clmmpicom, ier)
