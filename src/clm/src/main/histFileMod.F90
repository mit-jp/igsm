...
! !USES:
    ...
    use spmdMod   , only : masterproc, clmmpicom, CLMMPI_REAL8, CLMMPI_INTEGER, CLMMPI_CHARACTER
!
! !ARGUMENTS:
    implicit none
...
!------------------------------------------------------------------------
    ...
    if (masterproc) then
     ...
     if (flag == 'read') then
        ...
        call clmmpi_bcast (ntapes, 1, clmmpi_integer, 0, clmmpicom, ier)
        do t = 1,ntapes
           call clmmpi_bcast (tape(t)%nflds     , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
           call clmmpi_bcast (tape(t)%ntimes    , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
           call clmmpi_bcast (tape(t)%nhtfrq    , 1, CLMMPI_INTEGER, 0 ,clmmpicom, ier)
           call clmmpi_bcast (tape(t)%mfilt     , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
           call clmmpi_bcast (tape(t)%ncprec    , 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
           call clmmpi_bcast (tape(t)%is_endhist, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
           call clmmpi_bcast (tape(t)%dov2xy    , 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
           call clmmpi_bcast (tape(t)%begtime   , 1, CLMMPI_REAL8  , 0, clmmpicom, ier)
           do f = 1,tape(t)%nflds
              call clmmpi_bcast (tape(t)%hlist(f)%field%name          , max_namlen, CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%units         , max_chars , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%num2d         , 1         , CLMMPI_INTEGER  , 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%hpindex       , 1         , CLMMPI_INTEGER  , 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%type1d        , 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%type1d_out    , 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%type2d        , 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%typexy        , 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%nlonxy        , 1         , CLMMPI_INTEGER  , 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%p2c_scale_type, 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%c2l_scale_type, 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%l2g_scale_type, 8         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%field%nlatxy        , 1         , CLMMPI_INTEGER  , 0, clmmpicom, ier)
              call clmmpi_bcast (tape(t)%hlist(f)%avgflag             , 1         , CLMMPI_CHARACTER, 0, clmmpicom, ier)
           end do
        end do
     endif
