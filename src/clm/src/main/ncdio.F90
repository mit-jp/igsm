...
! !USES:
  ...
  use spmdMod        , only : masterproc, clmmpicom, CLMMPI_REAL8, CLMMPI_INTEGER, &
                              CLMMPI_LOGICAL
...
!cdir nodep
      ...
      if (masterproc) then
       ...
       call clmmpi_bcast(varpresent, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
       if (ier /= 0) then
          write(6,*)subname,' error from clmmpi_bcast'; call endrun()
       end if
...
!cdir nodep
       ...
       call clmmpi_bcast(varpresent, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
       if (ier /= 0) then
          write(6,*)trim(subname),' error from clmmpi_bcast'; call endrun()
       end if
...
!-----------------------------------------------------------------------
    ...
    if (flag == 'write') then
       ...
       call clmmpi_bcast(varpresent, 1, CLMMPI_LOGICAL, 0, clmmpicom, ier)
       if (ier /= 0) then
          write(6,*)trim(subname),' error from clmmpi_bcast for varpresent'; call endrun()
       end if
       if (varpresent) then
          call clmmpi_bcast(data, 1, CLMMPI_INTEGER, 0, clmmpicom, ier)
          if (ier /= 0) then
             write(6,*)trim(subname),' error from clmmpi_bcast for data'; call endrun()
          end if
       end if
       if (present(readvar)) readvar = varpresent
...
!-----------------------------------------------------------------------
    ...
    if (flag == 'write') then
       ...
       if (varpresent) then
          call clmmpi_bcast(data, 1, CLMMPI_REAL8, 0, clmmpicom, ier)
          if (ier /= 0) then
             write(6,*)trim(subname),' error from clmmpi_bcast for data'; call endrun()
          end if
       end if
...
!-----------------------------------------------------------------------
    ...
    if (flag == 'write') then
       ...
       if (varpresent) then
          call clmmpi_bcast(data, size(data), CLMMPI_INTEGER, 0, clmmpicom, ier)
          if (ier /= 0) then
             write(6,*)trim(subname),' error from clmmpi_bcast for data'; call endrun()
          end if
       end if
...
!-----------------------------------------------------------------------
    ...
    if (flag == 'write') then
       ...
       if (varpresent) then
          call clmmpi_bcast(data, size(data), CLMMPI_REAL8, 0, clmmpicom, ier)
          if (ier /= 0) then
             write(6,*)trim(subname),' error from clmmpi_bcast for data'; call endrun()
          end if
       end if
