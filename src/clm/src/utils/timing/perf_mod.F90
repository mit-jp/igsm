...
!-----------------------------------------------------------------------
   ...
   logical, private   :: timing_barrier = def_timing_barrier
                         ! flag indicating whether the clmmpi_barrier in
...
!========================================================================
!
   subroutine t_barrierf(event, clmmpicom)
!----------------------------------------------------------------------- 
! Purpose: Call (and time) clmmpi_barrier. Ignored inside OpenMP
!          threaded regions. Note that barrier executed even if
!          event not recorded because of level of timer event nesting.
! Author: P. Worley 
!-----------------------------------------------------------------------
!---------------------------Use statements------------------------------
   use shr_clmmpi_mod,       only: shr_clmmpi_barrier
!---------------------------Input arguments-----------------------------
   ! clmmpi communicator id
   integer, intent(in), optional :: clmmpicom
   ...
   if ((timing_initialized) .and. &
      ...
      if (timing_barrier) then
         ...
         if ( present (clmmpicom) ) then
            call shr_clmmpi_barrier(clmmpicom, 'T_BARRIERF: bad clmmpi communicator')
         else
            call shr_clmmpi_barrier(CLMMPI_COMM_WORLD, 'T_BARRIERF: bad clmmpi communicator')
         endif
...
!========================================================================
!
   subroutine t_prf(filename, clmmpicom)
!----------------------------------------------------------------------- 
...
!$OMP MASTER
   call clmmpi_comm_rank(CLMMPI_COMM_WORLD, gme, ierr)
   if ( present(clmmpicom) ) then
      call clmmpi_comm_size(clmmpicom, npes, ierr)
!pw      if (ierr .eq. CLMMPI_ERR_COMM) then
!pw         call shr_sys_abort('T_PRF: bad clmmpi communicator')
!pw      endif
      call clmmpi_comm_rank(clmmpicom, me, ierr)
   else
      call clmmpi_comm_size(CLMMPI_COMM_WORLD, npes, ierr)
      me = gme
   endif
 ...
 101     format(/,"************ PROCESS ",I5," (",I5,") ************",/)
         ...
         call clmmpi_recv (signal, 1, clmmpi_integer, me-1, me-1, clmmpicom, status, ierr)
         if (ierr /= clmmpi_success) then
            write(6,*) 'T_PRF: clmmpi_recv failed ierr=',ierr
            call shr_sys_abort()
         end if
      ...
      if (me+1 < npes) &
         call clmmpi_send (signal, 1, clmmpi_integer, me+1, me, clmmpicom, ierr)
...
!========================================================================
!
   subroutine t_initf(NLFilename, LogPrint, clmmpicom, MasterTask)
!----------------------------------------------------------------------- 
...
!$OMP MASTER
    if ( present(MasterTask) .and. present(clmmpicom) )then
       call clmmpi_comm_rank(clmmpicom, me, ierr)
!pw      if (ierr .eq. CLMMPI_ERR_COMM) then
!pw         call shr_sys_abort('T_INITF: bad clmmpi communicator')
!pw      endif
       if (me .eq. 0) then
          MasterTask2 = .true.
       else
    ...
    if ( present(MasterTask) .and. present(clmmpicom) )then
       call shr_clmmpi_bcast( profile_disable,    CLMMPICom )
       call shr_clmmpi_bcast( profile_barrier,    CLMMPICom )
       call shr_clmmpi_bcast( profile_single_file,  CLMMPICom )
       call shr_clmmpi_bcast( profile_depth_limit, CLMMPICom )
       call shr_clmmpi_bcast( profile_detail_limit, CLMMPICom )
       call shr_clmmpi_bcast( profile_timer,        CLMMPICom )
    end if
