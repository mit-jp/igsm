...
{
#ifdef SPMD
  double wtime1, wtime2;        /* returned from CLMMPI_Wtime() */
#else
  ...
  ** of sec as returned by CLMMPI_Wtime or sec and usec as returned by gettimeofday(). 
  */
...
int GPTLstop (const char *name) /* timer name */
{
  ...
  */
  ...
  if (wallstats.enabled) {
    ...
    ** Under SPMD, CLMMPI_Wtime is being used instead of gettimeofday().
...
#ifdef SPMD
  CLMMPI_Comm comm_c;         /* CLMMPI communicator */
  int tag;                 /* send/recv tag */
  int destid, srcid;       /* process ids */
  int err;                 /* CLMMPI error return */
  int signal=1;           /* signaling buffer */
  CLMMPI_Status status;       /* CLMMPI status return */
#endif
