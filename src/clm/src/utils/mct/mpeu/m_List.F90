...
!BOP -------------------------------------------------------------------
!
! !IROUTINE: bcast_ - CLMMPI Broadcast for the List Type
!
 ...
 type(String) :: DummStr
  ...
  if(ierr /= 0) then
   if(present(status)) then
     status = ierr
     write(stderr,'(2a,i4)') myname_,":: CLMMPI_COMM_RANK(), ierr=",ierr
     return
   else
...
!BOP -------------------------------------------------------------------
!
! !IROUTINE: send_ - CLMMPI Point-to-Point Send for the List Type
!
! !DESCRIPTION:  This routine takes an input {\tt List} argument 
...
! given CLMMPI tag value {\tt TagBase}.  The second send is the 
! {\tt CHARACTER} data present in {\tt inList\%bf}, and is given CLMMPI 
...
!EOP ___________________________________________________________________
 ...
 call CLMMPI_SEND(length, 1, MP_type(length), dest, TagBase, comm, ierr)
  if(ierr /= 0) then
     if(present(status)) then
        write(stderr,'(2a,i8)') myname_, &
	     ':: CLMMPI_SEND(length...) failed.  ierror=', ierr
        status = ierr
        return
     else
        call MP_perr_die(myname_,':: CLMMPI_SEND(length...) failed',ierr)
     endif
  endif
 ...
 call CLMMPI_SEND(DummStr%c(1), length, MP_CHARACTER, dest, TagBase+1, &
               comm, ierr)
  if(ierr /= 0) then
     if(present(status)) then
        write(stderr,'(2a,i8)') myname_, &
	     ':: CLMMPI_SEND(DummStr%c...) failed.  ierror=', ierr
        status = ierr
        return
     else
        call MP_perr_die(myname_,':: CLMMPI_SEND(DummStr%c...) failed',ierr)
     endif
  endif
...
!BOP -------------------------------------------------------------------
!
! !IROUTINE: recv_ - CLMMPI Point-to-Point Receive for the List Type
!
...
! is given CLMMPI tag value {\tt TagBase}.  The second receive is the 
! {\tt CHARACTER} data present in {\tt outList\%bf}, and is given CLMMPI 
! tag value {\tt TagBase+1}.
!
! !INTERFACE:
...
! !REVISION HISTORY:
!  6Jun01 - J.W. Larson - initial version.
! 11Jun01 - R. Jacob - small bug fix; status in CLMMPI_RECV
! 13Jun01 - J.W. Larson <larson@mcs.anl.gov> - Initialize status
!           (if present).
!EOP ___________________________________________________________________
 ...
 call CLMMPI_RECV(length, 1, MP_type(length), source, TagBase, comm, &
               MPstatus, ierr)
  if(ierr /= 0) then
     if(present(status)) then
        write(stderr,'(2a,i8)') myname_, &
	     ':: CLMMPI_RECV(length...) failed.  ierror=', ierr
        status = ierr
        return
     else
        call MP_perr_die(myname_,':: CLMMPI_RECV(length...) failed',ierr)
     endif
  endif
 ...
 call CLMMPI_RECV(DummStr%c(1), length, MP_CHARACTER, source, TagBase+1, &
               comm, MPstatus, ierr)
  if(ierr /= 0) then
     if(present(status)) then
        write(stderr,'(2a,i8)') myname_, &
	     ':: CLMMPI_RECV(DummStr%c...) failed.  ierror=', ierr
        status = ierr
        return
     else
        call MP_perr_die(myname_,':: CLMMPI_RECV(DummStr%c...) failed',ierr)
     endif
  endif
