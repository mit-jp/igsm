...
!BOP -------------------------------------------------------------------
!
! !IROUTINE: bcast_ - CLMMPI Broadcast of a rank-0 String
!
! !DESCRIPTION:
! This routine performs an CLMMPI broadcast of the input/output {\tt String}
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_bcast(ln,1,MP_INTEGER,root,comm,ier)
  if(ier/=0) then
  ...
  call CLMMPI_bcast(Str%c(1),ln,MP_CHARACTER,root,comm,ier)
  if(ier/=0) then
