...
! of operations supported by CLMMPI (currently, summation, minimum and 
...
! of all its attributes across the CLMMPI communicator associated with 
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_COMM_RANK(comm, myID, ier)
  if(ier /= 0) then
     write(stderr,'(2a)') myname_,':: CLMMPI_COMM_RANK() failed.'
     call MP_perr_die(myname_, 'MPI_COMM_RANK() failed.', ier)
  endif

  call AttrVect_init(outAV, inAV, lsize=AttrVect_lsize(inAV))

  if(List_allocated(inAV%rList)) then ! invoke CLMMPI_AllReduce() for the real
                                      ! attribute data.
     BufferSize = AttrVect_lsize(inAV) * AttrVect_nRAttr(inAV)

     select case(ReductionOp)
     case(AttrVectSUM)
	call CLMMPI_AllReduce(inAV%rAttr, outAV%rAttr, BufferSize, &
	...
	write(stderr,*) myname_, &
	     ':: Fatal Error in CLMMPI_AllReduce(), myID = ',myID
	call MP_perr_die(myname_, 'MPI_AllReduce() failed.', ier)
     endif

  endif ! if(List_allocated(inAV%rList))...

  if(List_allocated(inAV%iList)) then ! invoke CLMMPI_AllReduce() for the 
     ...
     select case(ReductionOp)
     case(AttrVectSUM)
	call CLMMPI_AllReduce(inAV%iAttr, outAV%iAttr, BufferSize, &
...
! of all the result of the local reduction across the CLMMPI communicator 
...
! a weighted global sum across the CLMMPI communicator associated with 
...
!EOP ___________________________________________________________________
  ...
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: CLMMPI_COMM_RANK() error.',ierr)
  endif
