...
! Attribute Vector between two components on separate sets of CLMMPI processes.
...
!           CLMMPI_Reqest and CLMMPI_Status arrays to Router.  Use them.
...
! 18May01 - R. Jacob <jacob@mcs.anl.gov> - use MP_Type to determine type in clmmpi_send
! 07Jun01 - R. Jacob <jacob@mcs.anl.gov> - remove logic to check "direction" of Router.  
!           remove references to ThisMCTWorld%mylrank
! 03Aug01 - E. Ong <eong@mcs.anl.gov> - Explicitly specify the starting address in clmmpi_send.  
...
! Return if nothing to wait for
  ...
  if(Rout%numiatt .ge. 1) then
     ...
     call CLMMPI_WAITALL(Rout%nprocs,Rout%ireqs,Rout%istatus,ier)
  ...
  if(Rout%numratt .ge. 1) then
     ...
     call CLMMPI_WAITALL(Rout%nprocs,Rout%rreqs,Rout%rstatus,ier)
...
!
! Will return as soon as CLMMPI\_IRECV's are posted.  Call {\tt waitrecv\_} to
...
! allocate buffers to hold all incoming data
  ...
  do proc=1,Rout%nprocs
	...
	if( Rout%num_segs(proc) > 1 .or. DoSum ) then
	   ...
	   call CLMMPI_IRECV(Rout%ip1(proc)%pi(1), &
		Rout%locsize(proc)*numi,MP_INTEGER,Rout%pe_list(proc), &
		mytag,ThisMCTWorld%MCT_comm,Rout%ireqs(proc),ier)

	else

	   call CLMMPI_IRECV(aV%iAttr(1,Rout%seg_starts(proc,1)), &
		Rout%locsize(proc)*numi,MP_INTEGER,Rout%pe_list(proc), &
		mytag,ThisMCTWorld%MCT_comm,Rout%ireqs(proc),ier)
	...
	if( Rout%num_segs(proc) > 1 .or. DoSum ) then
	   ...
	   call CLMMPI_IRECV(Rout%rp1(proc)%pr(1), &
		Rout%locsize(proc)*numr,mp_Type_rp1,Rout%pe_list(proc), &
		mytag,ThisMCTWorld%MCT_comm,Rout%rreqs(proc),ier)

	else

	   call CLMMPI_IRECV(aV%rAttr(1,Rout%seg_starts(proc,1)), &
		Rout%locsize(proc)*numr,mp_Type_rp1,Rout%pe_list(proc), &
		mytag,ThisMCTWorld%MCT_comm,Rout%rreqs(proc),ier)
...
!     if(Rout%numiatt .ge. 1) then
!	call CLMMPI_WAITANY(Rout%nprocs,Rout%ireqs,proc,Rout%istatus,ier)
...
!     ! Load the real data
!     if(numr .ge. 1) then
!	call CLMMPI_WAITANY(Rout%nprocs,Rout%rreqs,proc,Rout%rstatus,ier)
