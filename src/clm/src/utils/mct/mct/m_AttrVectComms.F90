...
!BOP -------------------------------------------------------------------
!
! !MODULE: m_AttrVectComms - CLMMPI Communications Methods for the AttrVect
!
! !DESCRIPTION:
!
...
! in terms of the Message Passing Interface (CLMMPI) standard, and we have 
! as best as possible, made the interfaces to these routines appear as
! similar as possible to the corresponding CLMMPI routines.  For the 
...
!           so its communication model agrees with CLMMPI_scatter().
...
! associated with the Fortran {\tt INTEGER} CLMMPI communicator handle 
...
!
! {\bf N.B.}:  One must avoid assigning elsewhere the CLMMPI tag values 
...
!EOP ___________________________________________________________________
 ...
 integer :: AVlength
  ...
  call CLMMPI_SEND(ListAssoc, 2, MP_LOGICAL, dest, TagBase, comm, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: CLMMPI_SEND(ListAssoc...',ierr)
  endif
  ...
  call CLMMPI_SEND(AVlength, 1, MP_type(AVlength), dest, TagBase+5, &
                comm, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: call CLMMPI_SEND(AVlength...',ierr)
  endif
  ...
  if(AVlength > 0) then
	...
	call CLMMPI_SEND(inAV%iAttr(1,1), AVlength*List_nitem(inAV%iList), &
                      MP_type(inAV%iAttr(1,1)), dest, TagBase+6,   &
                      comm, ierr)
	if(ierr /= 0) then
	   call MP_perr_die(myname_,':: call CLMMPI_SEND(inAV%iAttr...',ierr)
	endif
	...
	call CLMMPI_SEND(inAV%rAttr(1,1), AVlength*List_nitem(inAV%rList), &
                      MP_type(inAV%rAttr(1,1)), dest, TagBase+7,   &
                      comm, ierr)
	if(ierr /= 0) then
	   call MP_perr_die(myname_,':: call CLMMPI_SEND(inAV%rAttr...',ierr)
	endif
...
! with the Fortran {\tt INTEGER} CLMMPI communicator handle {\tt comm}.  The 
...
!EOP ___________________________________________________________________
 ...
 integer :: MPstatus(MP_STATUS_SIZE)
  ...
  call CLMMPI_RECV(ListAssoc, 2, MP_LOGICAL, dest, TagBase, comm, &
                MPstatus, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: CLMMPI_RECV(ListAssoc...',ierr)
  endif
  ...
  call CLMMPI_RECV(AVlength, 1, MP_type(AVlength), dest, TagBase+5, &
                comm, MPstatus, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_,':: call CLMMPI_RECV(AVlength...',ierr)
  endif
  ...
  if(AVlength > 0) then
	...
	call CLMMPI_RECV(outAV%iAttr(1,1), AVlength*List_nitem(outAV%iList), &
                      MP_type(outAV%iAttr(1,1)), dest, TagBase+6,   &
                      comm, MPstatus, ierr)
	if(ierr /= 0) then
	   call MP_perr_die(myname_,':: call CLMMPI_RECV(outAV%iAttr...',ierr)
	endif
	...
	call CLMMPI_RECV(outAV%rAttr(1,1), AVlength*List_nitem(outAV%rList), &
                      MP_type(outAV%rAttr(1,1)), dest, TagBase+7,   &
                      comm, MPstatus, ierr)
	if(ierr /= 0) then
	   call MP_perr_die(myname_,':: call CLMMPI_RECV(outAV%rAttr...',ierr)
	endif
...
! argument {\tt comm} is the Fortran integer CLMMPI communicator handle.
...
!           to determine type for clmmpi_gatherv
!EOP ___________________________________________________________________
  ...
  if(nIA > 0) then
     ...
     if(myID == root) then
        ...
        call CLMMPI_gatherv(iV%iAttr(1,1),niV*nIA,MP_INTEGER,		&
             oV%iAttr(1,1),GMap%counts*nIA,GMap%displs*nIA,             &
             MP_INTEGER,root,comm,ier)
        if(ier /= 0) then
           call MP_perr_die(myname_,':: CLMMPI_gatherv(iAttr) on root',ier)
        endif
        ...
        call CLMMPI_gatherv(iV%iAttr(1,1),niV*nIA,MP_INTEGER,		&
             ...
             MP_INTEGER,root,comm,ier)
        if(ier /= 0) then
           call MP_perr_die(myname_,':: CLMMPI_gatherv(iAttr) off root',ier)
        endif
  ...
  if(nRA > 0) then
     ...
     if(myID == root) then
        ...
        call CLMMPI_gatherv(iV%rAttr(1,1),niV*nRA,mp_Type_iV,	        &
             oV%rAttr(1,1),GMap%counts*nRA,GMap%displs*nRA,             & 
             mp_Type_oV,root,comm,ier)
        if(ier /= 0) then
           call MP_perr_die(myname_,':: CLMMPI_gatherv(rAttr) on root',ier)
        endif
        ...
        call CLMMPI_gatherv(iV%rAttr(1,1),niV*nRA,mp_Type_iV,	        &
             ...
             mp_Type_oV,root,comm,ier)
        if(ier /= 0) then
           call MP_perr_die(myname_,':: CLMMPI_gatherv(rAttr) off root',ier)
        endif
...
! workV sizes
  ...
  call CLMMPI_COMM_RANK(comm, myID, ierr)

  if(ierr /= 0) then
	call MP_perr_die(myname_,':: call CLMMPI_COMM_RANK()',ierr)
  endif
       ! How many processes are there on this communicator?

  call CLMMPI_COMM_SIZE(comm, NumProcs, ierr)

  if(ierr /= 0) then
     call MP_perr_die(myname_,':: call CLMMPI_COMM_SIZE()',ierr)
  endif
...
!           empty calls (i.e. no data in buffer) to CLMMPI_SCATTERV()
...
!           to reflect CLMMPI comms model for the scatter.  Tidied up
!           the prologue, too.
! 18May01 - R.L. Jacob <jacob@mcs.anl.gov> - use MP_Type function
!           to determine type for clmmpi_scatterv
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_BCAST(nIA,1,MP_INTEGER,root,comm,ier)
  if(ier /= 0) call MP_perr(myname_,'MPI_BCAST(nIA)',ier)

  call CLMMPI_BCAST(nRA,1,MP_INTEGER,root,comm,ier)
  ...
  call AttrVect_init(oV, iList, rList, noV)
        ...
        ! Initialize a dummy AttrVect for non-root CLMMPI calls
  ...
  if(nIA > 0) then
     ...
     if(myID == root) then
        ...
        call CLMMPI_scatterv(iV%iAttr(1,1),GMap%counts*nIA,	&
        ...
        call CLMMPI_scatterv(nonRootAV%iAttr(1,1),GMap%counts*nIA,	&
             GMap%displs*nIA,MP_INTEGER,oV%iAttr(1,1),          &
             noV*nIA,MP_INTEGER,root,comm,ier )
        if(ier /= 0) then
  ...
  if(nRA > 0) then
     ...
     if(myID == root) then
        ...
        call CLMMPI_scatterv(iV%rAttr(1,1),GMap%counts*nRA,	&
        ...
        call CLMMPI_scatterv(nonRootAV%rAttr(1,1),GMap%counts*nRA,	&
             GMap%displs*nRA,mp_Type_iV,oV%rAttr(1,1),          &
             noV*nRA,mp_Type_oV,root,comm,ier )
        if(ier /= 0) then
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_bcast(lsize,1,MP_INTEGER,root,comm,ier)
  if(ier /= 0) then
  ...
  if(nIA > 0) then
     ...
     call CLMMPI_bcast(aV%iAttr,nIA*lsize,mp_Type_aV,root,comm,ier)
     if(ier /= 0) then
  ...
  if(nRA > 0) then
     ...
     call CLMMPI_bcast(aV%rAttr,nRA*lsize,mp_Type_aV,root,comm,ier)
     if(ier /= 0) then
