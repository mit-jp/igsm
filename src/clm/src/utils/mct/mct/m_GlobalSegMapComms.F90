...
! are provided for send (analogues to {\tt CLMMPI\_SEND()/MPI\_ISEND()})
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_SEND(outgoingGSMap%comp_id, 1, MP_Type(outgoingGSMap%comp_id), destID, &
                TagBase, ThisMCTWorld%MCT_comm, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_, 'Send compid failed',ierr)
  endif

  call CLMMPI_SEND(outgoingGSMap%ngseg, 1, MP_Type(outgoingGSMap%ngseg), destID, &
                TagBase+1, ThisMCTWorld%MCT_comm, ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_, 'Send ngseg failed',ierr)
  endif

  call CLMMPI_SEND(outgoingGSMap%gsize, 1, MP_Type(outgoingGSMap%gsize), destID, &
  ...
  call CLMMPI_SEND(outgoingGSMap%start, nsegs, &
  ...
  call CLMMPI_SEND(outgoingGSMap%length, nsegs, &
  ...
  call CLMMPI_SEND(outgoingGSMap%pe_loc, nsegs, &
...
! using any of CLMMPI's wait functions.  The success (failure) of 
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_ISEND(outgoingGSMap%comp_id, 1, MP_Type(outgoingGSMap%comp_id), destID, &
                TagBase, ThisMCTWorld%MCT_comm, reqHandle(1), ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_, 'Send compid failed',ierr)
  endif

  call CLMMPI_ISEND(outgoingGSMap%ngseg, 1, MP_Type(outgoingGSMap%ngseg), destID, &
                TagBase+1, ThisMCTWorld%MCT_comm, reqHandle(2), ierr)
  if(ierr /= 0) then
     call MP_perr_die(myname_, 'Send ngseg failed',ierr)
  endif

  call CLMMPI_ISEND(outgoingGSMap%gsize, 1, MP_Type(outgoingGSMap%gsize), destID, &
  ...
  call CLMMPI_ISEND(outgoingGSMap%start, nsegs, &
                MP_Type(outgoingGSMap%start(1)), &
  ...
  call CLMMPI_ISEND(outgoingGSMap%length, nsegs, &
                MP_Type(outgoingGSMap%length(1)), &
  ...
  call CLMMPI_ISEND(outgoingGSMap%pe_loc, nsegs, &
                MP_Type(outgoingGSMap%pe_loc(1)), &
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_RECV(incomingGSMap%start, RecvBuffer(2), &
  ...
  call CLMMPI_RECV(incomingGSMap%length, RecvBuffer(2), &
  ...
  call CLMMPI_RECV(incomingGSMap%pe_loc, RecvBuffer(2), &
...
!EOP ___________________________________________________________________
  ...
  call CLMMPI_BCAST(IntBuffer, 3, MP_type(IntBuffer(1)), root, comm, ierr)
  ...
  call CLMMPI_BCAST(IntBuffer, 3*GSMap%ngseg, MP_Type(IntBuffer(1)), root, comm, ierr)
  if(ierr /= 0) call MP_perr_die(myname_,'Error in second CLMMPI_BCAST(IntBuffer)',ierr)
