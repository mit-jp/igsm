...
! !PRIVATE DATA MEMBERS:
      integer :: max_nprocs  ! size of CLMMPI_COMM_WORLD used for generation of
...
!-----------------------------------------------------------------------
   ...
   ! DECLARE STRUCTURES FOR CLMMPI ARGUMENTS.
   ...
   ! Structure to hold CLMMPI request information for sends
   ...
   ! Structure to hold CLMMPI status information for sends 
  ...
  do pe_shift = 1,max_pe
   proc = RecvList(mod(myPid+pe_shift,max_pe))
	...
	if( (RecvRout%num_segs(proc) > 1) .or. DoSum ) then
	   ...
	   call CLMMPI_IRECV(IRecvBuf(IRecvLoc(proc)),                 &
	   ...
	   call CLMMPI_IRECV(TargetAV%iAttr(1,RecvRout%seg_starts(proc,1)), &
		          RecvRout%locsize(proc)*numi,MP_INTEGER,        &
                          RecvRout%pe_list(proc),mytag,                  &
                          ThisMCTWorld%MCT_comm,recv_ireqs(proc),ier)
	...
	if( (RecvRout%num_segs(proc) > 1) .or. DoSum ) then
	   ...
	   call CLMMPI_IRECV(RRecvBuf(RRecvLoc(proc)),                 &
	   ...
	   call CLMMPI_IRECV(TargetAV%rAttr(1,RecvRout%seg_starts(proc,1)), &
		          RecvRout%locsize(proc)*numr,mp_Type_rp,        &
			  RecvRout%pe_list(proc),mytag,                  &
			  ThisMCTWorld%MCT_comm,recv_rreqs(proc),ier)
  ...
  do pe_shift = max_pe,1,-1
   proc = SendList(mod(myPid+pe_shift,max_pe))
	...
	if( SendRout%num_segs(proc) > 1 ) then
	   ...
	   call CLMMPI_ISEND(ISendBuf(ISendLoc(proc)),                 &
	   ...
	   call CLMMPI_ISEND(SourceAV%iAttr(1,SendRout%seg_starts(proc,1)), &
		          SendRout%locsize(proc)*numi,MP_INTEGER,        &
                          SendRout%pe_list(proc),mytag,                  &
                          ThisMCTWorld%MCT_comm,send_ireqs(proc),ier)
	...
	if( SendRout%num_segs(proc) > 1 ) then
	   ...
	   call CLMMPI_ISEND(RSendBuf(RSendLoc(proc)),                 &
	   ...
	   call CLMMPI_ISEND(SourceAV%rAttr(1,SendRout%seg_starts(proc,1)), &
		          SendRout%locsize(proc)*numr,mp_Type_rp,        &
			  SendRout%pe_list(proc),mytag,                    &
			  ThisMCTWorld%MCT_comm,send_rreqs(proc),ier)
...
if (usealltoall) then
  ...
  if (numi .ge. 1) then
     call CLMMPI_Alltoallv(ISendBuf, ISendCnts, ISdispls, MP_INTEGER, &
                        IRecvBuf, IRecvCnts, IRdispls, MP_INTEGER, &
                        ThisMCTWorld%MCT_comm,ier)
  endif

  if (numr .ge. 1) then
     call CLMMPI_Alltoallv(RSendBuf, RSendCnts, RSdispls, mp_Type_rp, &
  ...
  if(SendRout%nprocs > 0) then
	...
	call CLMMPI_WAITALL(SendRout%nprocs,send_ireqs,send_istatus,ier)
	...
	call CLMMPI_WAITALL(SendRout%nprocs,send_rreqs,send_rstatus,ier)
...
if (usealltoall) then
        ...
        if(DoSum) then
           proc = numprocs
	   call CLMMPI_WAIT(recv_ireqs(proc),recv_istatus,ier)
        else
	   call CLMMPI_WAITANY(RecvRout%nprocs,recv_ireqs,proc,recv_istatus,ier)
        endif
endif
...
if (usealltoall) then
	...
	if(DoSum) then
           proc = numprocs
	   call CLMMPI_WAIT(recv_rreqs(proc),recv_rstatus,ier)
        else
	   call CLMMPI_WAITANY(RecvRout%nprocs,recv_rreqs,proc,recv_rstatus,ier)
        endif
endif
