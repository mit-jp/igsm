...
! !USES:
  ...
  use spmdMod     , only : masterproc,npes,iam,clmmpicom,comp_id,CLMMPI_REAL8
...
10     format(1x,a16,f7.2,1x,f7.2,a30)
    ...
    call clmmpi_bcast(gfrac,size(gfrac),CLMMPI_REAL8,0,clmmpicom,ier)
   ...
   !--- nocn = number of source gridcells for ocean gridcell
    ...
    call shr_sys_flush(6)
#endif
    call clmmpi_barrier(clmmpicom,ier)
    npmin = 0
    npmax = npes-1
    npint = 1
    if (dbug == 0) then
    ...
    enddo
    dtover = dtovermax
    call clmmpi_allreduce(dtover,dtovermax,1,CLMMPI_REAL8,CLMMPI_MAX,clmmpicom,ier)
   ...
   !--- initialization rtm gsmap
    ...
    call mct_gsMap_init( gsMap_rtm_gdc2glo, gindex, clmmpicom, comp_id, lsize, gsize )
    deallocate(gindex)
...
!-----------------------------------------------------------------------
    ...
    if (do_rtm) then
       ...
       ns = mct_gsMap_lsize(gsmap_lnd_gdc2glo, clmmpicom)
       call mct_aVect_init(aV_lndr,rlist='rtminput',lsize=ns)
       ns = mct_gsMap_lsize(gsMap_rtm_gdc2glo, clmmpicom)
...
!-----------------------------------------------------------------------
    ...
    call t_barrierf('sync_clmrtm', clmmpicom)
