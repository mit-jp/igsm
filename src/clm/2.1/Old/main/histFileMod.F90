...
!-----------------------------------------------------------------------
    ...
    call masterlist_addfld (fname='STORMS', type1d='gridcell', units='# of events', numlev=1, &
         avgflag='I', long_name='# of storms', hpindices=hpindices)
#endif
...
! !USES
    ...
    use clm_varpar, only : lsmlon, lsmlat, nlevsoi
#if (defined COUP_TEM)
    use clmtype, only : grid1d,cols1d,pfts1d
...
!      if (tape(t)%hlist(f)%field%name == fldlst(24)) rstrmdry1d => tape(t)%hlist(f)%hbuf(:,1)
...
!          xstrmadj(jj)=xstrmadj(jj)+exstrm
        endif
...
!         mitstorm(day,j)=rstorms1d(jj)-xstrmadj(jj)
!         if (j==23) then
!           write(6,*) 'day',day,'raw storms',rstorms1d(jj)
!           write(6,*) 'day',day,'storm adj',xstrmadj(jj)
!         endif
       endif
    enddo !jj
   enddo
...
!
!    write(6,*) 'day',day,'strmdry',mitstrmdry(day,23)
...
!       if (t == 3) then
!          tape(t)%is_endhist = .false.
!          if (mod(nstep+1,tape(t)%nhtfrq) == 0) tape(t)%is_endhist = .true.
!       end if
!#endif
