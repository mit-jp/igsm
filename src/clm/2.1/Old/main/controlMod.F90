...
!    o fstochdat       = 256 character stochastic data file name
...
!    o hist_crtinic  = 8  character frequency to generate initial dataset ['MONTHLY','YEARLY','NONE']
...
! ----------------------------------------------------------------------
    ...
    if (masterproc) then
       ...
       open(155,file='clm.nml',status='unknown')
       read(155,clmexp, iostat=ierr)
#else
